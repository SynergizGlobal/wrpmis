...
public class SectionDaoImpl implements SectionDao{
	...
	public boolean addSection(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO sections"
					+ "( section_id,section_name,division_id) VALUES (:section_id, :section_name, :division_id)";
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as section_id,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from ["+bObj.getTable_name()+"] where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					String qry2 = "select section_id,section_name,division_id from sections where section_id NOT IN (?";
		
					Object[] pValues = new Object[objsList1.size()];
					int j =0, p=1;
					for (TrainingType aObj : obj.getdList()) {
						pValues[j++] = aObj.getSection_id();
						if( objsList1.size() >  p) {
							qry2 = qry2 + ",?";
							p++;
						}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='Section' and s.name='dbo'\r\n" + 
	...
	public boolean updateSection(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table sections NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE sections SET section_id= :section_id_new,section_name= :section_name_new,division_id= :division_id_new WHERE section_id= :section_id_old " ;
			paramSource = new BeanPropertySqlParameterSource(obj);		 
			count = namedParamJdbcTemplate.update(updatereferenceTableQry, paramSource);	
			
			for (TrainingType bObj : obj.getdList()) {
				
				String updateTableQry = "UPDATE "+bObj.getTable_name()+" SET "+bObj.getColumn_name()+" =:section_id_new WHERE "+bObj.getColumn_name()+"= :section_id_old " ;
				
				 paramSource = new BeanPropertySqlParameterSource(obj);		 
				 namedParamJdbcTemplate.update(updateTableQry, paramSource);	
			}
			String  enableQry =	"Alter Table sections CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteSection(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE FROM sections WHERE section_id= :section_id; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
