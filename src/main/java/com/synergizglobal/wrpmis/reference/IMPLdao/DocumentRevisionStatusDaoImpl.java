...
import com.synergizglobal.wrpmis.reference.Idao.DocumentRevisionStatusDao;
import com.synergizglobal.wrpmis.reference.model.Safety;
import com.synergizglobal.wrpmis.reference.model.TrainingType;
@Repository
public class DocumentRevisionStatusDaoImpl implements DocumentRevisionStatusDao{
	...
	public List<Safety> getDocumentRevisionStatusList() throws Exception {
		...
		try {
			String qry ="select status from documents_revisions_status ";
	...
	public boolean addDocumentRevisionStatus(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO documents_revisions_status"
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select status from documents_revisions_status where status NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='documents_revisions_status' and s.name='dbo'\r\n" + 
	...
	public boolean updateDocumentRevisionStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table documents_revisions_status NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE documents_revisions_status SET status= :status_new WHERE status= :status_old " ;
			...
			String  enableQry =	"Alter Table documents_revisions_status CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteDocumentRevisionStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from documents_revisions_status WHERE status= :status; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
