...
import com.synergizglobal.wrpmis.reference.Idao.UtilityResponsibleExecutivesDao;
...
public class UtilityResponsibleExecutivesDaoImpl implements UtilityResponsibleExecutivesDao{
	...
	public List<TrainingType> getExecutivesDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT  distinct p.project_id,p.project_id as project_id_fk, project_name, STRING_AGG(u.user_name , ',') user_name,STRING_AGG(u.user_id , ',') user_id FROM utility_shifting_executives re "
					+ "LEFT JOIN [user] u on re.executive_user_id_fk = u.user_id "
					+ "left join project p on re.project_id_fk = p.project_id group by p.project_id,project_name ";
