...
<body>
                        ...
                        <div class="row no-mar">
                            <div class="col s12 center-align">
                                <a class="waves-effect waves-light btn bg-s modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add As Built Status</a>
                            </div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="as_built_status_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
										<c:forEach var="obj" items="${asBuiltStatusDetails.dList1}" varStatus="indexs">
											<tr><td>
												<input type="hidden" id="as_built_statusId${indexs.count}" value="${obj.as_built_status }" class="findLengths"/>
											...
											<c:forEach var="tObj" items="${asBuiltStatusDetails.tablesList}" varStatus="index">
												...
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    		...
													    		<c:choose>  
																    <c:when test="${cObj.as_built_status eq obj.as_built_status }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
										 	...
										 	<c:forEach var="oSbj"  items="${asBuiltStatusDetails.dList}" varStatus="indexx"> 
												...
												<c:choose>  
												    <c:when test="${oSbj.as_built_status eq obj.as_built_status }"> 
												      	<a onclick="deleteRow('${ oSbj.as_built_status }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
    ...
    <div id="addUpdateModal" class="modal">
                    ...
                    <div class="col m8 s12">
                        <div class="row">
                            <div class="input-field col s12 m12">
                                <input id="as_built_status_text" name="as_built_status" type="text" class="validate" onkeyup="doValidate(this.value)" >
                                <label for="as_built_status_text">As Built Status</label>
                                <span id="as_built_statusError" class="error-msg" ></span>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col s6 m6">
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-as-built-status" id=updateAsBuiltStatusForm name="updateAsBuiltStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header bg-m">Update As Built Status <span class="right modal-action modal-close" onclick="removeErrorMsg()"><span
                            class="material-icons">close</span></span></h5>
                <div class="row">
                    <div class="col m2 hide-on-small"></div>
                    <div class="col m8 s12">
                       <div class="row no-mar">
                         <div class="input-field col s12 m12">
                                ...
                                <label for="value_new">As Built Status</label>
...
<%--   <jsp:include page="../layout/footer.jsp"></jsp:include> --%>
	<form name="getForm" id="getForm" method="post">
    	<input type="hidden" name="as_built_status" id="as_built_status" />
    </form>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table = $('#as_built_status_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
        ...
        function doValidate(value){
     	   ...
     	   while(count < validate){
     		   ...
     		   if(findVal == value){
     			   $('#as_built_statusError').text(print_value+' alreday exists').css('color', 'red');
     			   $('#bttn').prop('disabled', true);
     			   flag = false;
     			   return false;
     		   }else{
     			   $('#as_built_statusError').text('');
        ...
        function doValidateUpdate(value){
         ...
         $("#asBuiltStatusForm").submit(function (e) {
           	 if(validator.form()){ 
       			$(".page-loader").show();
       ...
       function removeErrorMsg(){
         ...
         $("#updateAsBuiltStatusForm").submit(function (e) {
        	 if(validator1.form()){ 
     			$(".page-loader").show();
        ...
        var validator =	$('#asBuiltStatusForm').validate({
       	 rules: {
        	 ...
        	 if(element.attr("id") == "as_built_status_text" ){
			     document.getElementById("as_built_statusError").innerHTML="";
		 	     error.appendTo('#as_built_statusError');
			 }
        }
      });
      var validator1 = $('#updateAsBuiltStatusForm').validate({
          	 rules: {
          		 	"value_new": {
      			 		  required: true
          			 }
        ...
        function updateRow(no) {
            var as_built_status = $('#as_built_statusId'+no).val();
            $('#value_old').val($.trim(as_built_status))
            $('#onlyUpdateModal').modal('open');
            $('#onlyUpdateModal #value_new').val($.trim(as_built_status)).focus();
        }
        
        function deleteRow(val){
        	$("#as_built_status").val(val);
        	showCancelMessage();
      	    }
