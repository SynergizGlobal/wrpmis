...
public class WbsTreeDaoImpl implements WbsTreeDao{
	...
	public List<WbsTree> getLevel4List(WbsTree obj) throws Exception {
		...
		try {
			String qry ="select wbs_4_name,wbs_3_name,wbs_2_name,contract_id,CAST(ifnull((sum(completed)/sum(scope))*100,0) AS DECIMAL(10,2)) as actual,CAST(ifnull(sum(weightage),0) AS DECIMAL(10,2)) as weightage, CASE  WHEN baseline_finish <= CURDATE() THEN 100 WHEN baseline_start > CURDATE() "
					+ " THEN 0 ELSE CAST((datediff(CURDATE() , baseline_start)+1)/(datediff(baseline_finish , baseline_start)+1) * 100 AS DECIMAL(10,2)) END as 'planned' " + 
					" from p6_view pv "
