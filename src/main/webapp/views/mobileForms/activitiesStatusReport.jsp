		...
		<div class="col s12 m12">
			<div class="card ">
				<div class="card-content">
					...
					<!-- form start-->
					<form action="<%=request.getContextPath()%>/mobileappwebview/generate-activities-status-report" id="activitiesReportForm" name="activitiesReportForm" method="post" target="_blank">
						<div class="row">
	...
	<script>
	 ...
	 var filtersMap = new Object();
        ...
        function clearFilters(){
        	...
        	window.location.href= "<%=request.getContextPath()%>/mobileappwebview/activities-status-report";
        }
      ...
      function getActivitiesList(){
        ...
        function getProjectsList(project) {
            ...
            if ($.trim(project_id) == "") {
            	...
            	var myParams = {};
                $.ajax({
	                url: "<%=request.getContextPath()%>/mobileappwebview/ajax/getProjectsFilterListInActivitiesStatusReport",
        ...
        function getWorksList(work) {
        	...
        	if(contract_id == ""){$("#fob_id_fk_div").hide();}
               ...
               $.ajax({
                url: "<%=request.getContextPath()%>/mobileappwebview/ajax/getWorksFilterListInActivitiesStatusReport",
                data: myParams, cache: false,async: false,
                success: function (data) {
                    if (data.length > 0) {
                        $.each(data, function (i, val) {
        ...
        function getContractsList(contarct) {
        	...
        	if(contract_id == ""){$("#fob_id_fk_div").hide();}
                ...
                $.ajax({
	            	url: "<%=request.getContextPath()%>/mobileappwebview/ajax/getContractsFilterListInActivitiesStatusReport",
	                data: myParams, cache: false,async: false,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) {
        ...
        function getFobList(fob) {
          ...
          //  if ($.trim(contract_id) != "" && $.trim(fob_id_fk) == "") {
                ...
                $.ajax({
                    url: "<%=request.getContextPath()%>/mobileappwebview/ajax/getFobFilterListInActivitiesStatusReport",
                    data: myParams, cache: false,async: false,
                    success: function (data) {
                        if (data.length > 0) {
                            $.each(data, function (i, val) {
