...
public class UtilityResponsibleExecutivesDaoImpl implements UtilityResponsibleExecutivesDao{
	...
	public List<TrainingType> getExecutivesDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT  work_id_fk, work_short_name, STRING_AGG(DISTINCT (u.user_name) SEPARATOR ',') user_name,STRING_AGG(DISTINCT (u.user_id) SEPARATOR ',') user_id FROM utility_shifting_executives re "
