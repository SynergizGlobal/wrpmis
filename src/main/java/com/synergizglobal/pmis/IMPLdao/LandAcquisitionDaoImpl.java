...
public class LandAcquisitionDaoImpl implements LandAcquisitionDao{
	...
	private String getAutoGeneratedLAId(LandAcquisition obj) {
		...
		LandAcquisition WorkCodedObj = null;
		String laId = obj.getWork_code()+"-LA-0001";
		List<LandAcquisition> objsList = null;
		try {
			String qry ="select work_code  from work where work_id='"+obj.getWork_id_fk()+"'" ;
			WorkCodedObj = (LandAcquisition)jdbcTemplate.queryForObject(qry, new BeanPropertyRowMapper<LandAcquisition>(LandAcquisition.class));	
...
*/
				String qry2="select CONCAT('"+WorkCodedObj.getWork_code()+"','-LA-',case when len(la_id)=3 then concat('0',la_id) when len(la_id)=2 then concat('00',la_id) when len(la_id)=1 then concat('000',la_id) end) as la_id from(" + 
						"select (case when (select count(*) from la_land_identification where left(la_id,2) ='"+WorkCodedObj.getWork_code()+"')>0 then Max(SUBSTRING( la_id , LEN(la_id) -  CHARINDEX('-',REVERSE(la_id)) + 2  , LEN(la_id)  ))+1 else 1 end )as la_id from la_land_identification where left(la_id,2) ='"+WorkCodedObj.getWork_code()+"') as a";
