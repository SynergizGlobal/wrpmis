...
import com.synergizglobal.pmis.reference.Idao.RRTenureStatusDao;
import com.synergizglobal.pmis.reference.model.TrainingType;

@Repository
public class RRTenureStatusDaoImpl implements RRTenureStatusDao{
	@Autowired
					...
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select rr_tenure_status from rr_tenure_status where rr_tenure_status NOT IN (?";
						...
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getRr_tenure_status();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select t1.name as tname,(select c2.name from sys.columns c2 where c2.object_id=t1.object_id\r\n" + 
					...
					"WHERE t.name = 'rr_tenure_status' and s.name = 'dbo'),'%')) as COLUMN_NAME,'' as CONSTRAINT_NAME,\r\n" + 
					"'rr_tenure_status' as REFERENCED_TABLE_NAME,\r\n" + 
					...
					"WHERE t2.name = 'rr_tenure_status' and s2.name = 'dbo')) as REFERENCED_COLUMN_NAME\r\n" + 
					...
					"left join sys.tables t on t.object_id = c.object_id\r\n" + 
					"left join sys.schemas s on s.schema_id = t.schema_id\r\n" + 
					"WHERE t.name = 'rr_tenure_status' and s.name = 'dbo'),'%'))\r\n" + 
					"and t1.name !=  'rr_tenure_status'";
	...
	public boolean addRRTenureStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO rr_tenure_status"
					+ "( rr_tenure_status) VALUES (:rr_tenure_status)";
