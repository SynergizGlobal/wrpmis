...
public class RRDocumentTypeDaoImpl implements RRDocumentTypeDao{
	...
	public List<Safety> getRRDocumentTypeList() throws Exception {
		...
		try {
			String qry ="select document_type from rr_document_type ";
	...
	public boolean addRRDocumentType(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO rr_document_type"
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select document_type from rr_document_type where `document_type` NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'rr_document_type' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'rr_document_type' and TABLE_SCHEMA = 'pmis' ";
	...
	public boolean updateRRDocumentType(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE rr_document_type SET `document_type`= :value_new WHERE `document_type`= :value_old " ;
	...
	public boolean deleteRRDocumentType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from rr_document_type WHERE `document_type`= :document_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
