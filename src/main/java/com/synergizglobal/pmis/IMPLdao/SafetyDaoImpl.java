	...
	public List<Safety> getSafetyList(Safety obj) throws Exception {
		...
		try {
			String qry = "SELECT safety_id,contract_id_fk,title,description,DATE_FORMAT(date,'%d-%m-%Y') AS date,location,latitude,longitude,reported_by,responsible_person,s.department_fk,"
					...
					+ "DATE_FORMAT(investigation_completed,'%d-%m-%Y') AS investigation_completed,corrective_measure_short_term,corrective_measure_long_term,compensation,DATE_FORMAT(payment_date,'%d-%m-%Y') AS payment_date,s.remarks,contract_name,work_id_fk,work_name,project_id_fk,project_name "
					+ "from safety s "
