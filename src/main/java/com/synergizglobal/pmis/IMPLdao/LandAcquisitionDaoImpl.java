...
public class LandAcquisitionDaoImpl implements LandAcquisitionDao{
	...
	@Override
	public LandAcquisition getLandAcquisitionForm(LandAcquisition obj) throws Exception {					//DATE_FORMAT(date_of_start,'%d-%m-%Y') AS cast(estimated_cost as CHAR) as
		...
		try {
			String qry = "select la_id,li.attachment, work_id_fk,w.project_id_fk,p.project_name,w.work_short_name, li.survey_number, li.village_id,c.la_category as type_of_land ,sc.la_sub_category as sub_category_of_land, li.village, taluka, dy_slr, sdo, collector, DATE_FORMAT(proposal_submission_date_to_collector,'%d-%m-%Y') AS proposal_submission_date_to_collector, cast(area_of_plot as CHAR) as area_of_plot, jm_fee_amount, cast(chainage_from as CHAR) as chainage_from,cast(chainage_to as CHAR) as chainage_to, DATE_FORMAT(jm_fee_letter_received_date,'%d-%m-%Y') AS jm_fee_letter_received_date,DATE_FORMAT(jm_fee_paid_date,'%d-%m-%Y') AS jm_fee_paid_date,DATE_FORMAT(jm_start_date,'%d-%m-%Y') AS  jm_start_date,DATE_FORMAT(jm_completion_date,'%d-%m-%Y') AS jm_completion_date, DATE_FORMAT(jm_sheet_date_to_sdo,'%d-%m-%Y') AS jm_sheet_date_to_sdo, jm_remarks, jm_approval, li.issues"
					...
					+ ",lg.id, lg.la_id_fk,cast(lg.area_to_be_acquired as CHAR) as area_to_be_acquired,DATE_FORMAT(lg.proposal_submission,'%d-%m-%Y') AS proposal_submission, lg.proposal_submission_status_fk, DATE_FORMAT(lg.valuation_date,'%d-%m-%Y') AS valuation_date, lg.letter_for_payment,"
					...
					+ "lg.possession_status_fk, lg.special_feature, lg.area_acquired,lg.remarks as gov_remarks, "
					...
					+ "lf.payment_status_fk as forest_payment_status_fk,lf.area_acquired as forest_area_acquired,lf.special_feature as forest_special_feature, lf.attachment_No as forest_attachment_No ,lf.remarks as forest_remarks"
					
					+ " ,lr.area_to_be_acquired as railway_area_to_be_acquired, lr.area_acquired as railway_area_acquired,DATE_FORMAT(lr.online_submission,'%d-%m-%Y') AS railway_online_submission,"
					...
					+ "lpa.forest_tree_rate, lpa.consent_from_owner, DATE_FORMAT(lpa.legal_search_report,'%d-%m-%Y') AS legal_search_report, DATE_FORMAT(lpa.date_of_registration,'%d-%m-%Y') AS date_of_registration, DATE_FORMAT(lpa.date_of_possession,'%d-%m-%Y') AS date_of_possession, lpa.possession_status_fk, lpa.special_feature as private_special_feature, "
					+ "lpa.area_acquired as private_area_acquired,cast(lpa.hundred_percent_Solatium as CHAR) as hundred_percent_Solatium,cast(lpa.extra_25_percent as CHAR) as extra_25_percent, cast(lpa.total_rate_divide_m2 as CHAR) as total_rate_divide_m2,cast(lpa.land_compensation as CHAR) as land_compensation, "
					...
					+ "cast(lpv.payment_amount as CHAR) as payment_amount, lpv.payment_date as private_payment_date ,lpv.remarks as private_remarks "
					+"from la_land_identification li "
