...
<body>
                        ...
                        <div class="row no-mar">
                            <div class="col s12 center-align">
                                <a class="waves-effect waves-light btn bg-s modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add Tenure Status</a>
                            </div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="rr_tenure_status_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                            <th> Tenure Status</th>
											...
											<c:forEach var="tObj" items="${TenureStatusDetails.tablesList}" varStatus="index">
												...
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    		...
													    		<c:choose>  
																    <c:when test="${cObj.rr_tenure_status eq obj.rr_tenure_status }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
										 	...
										 	<c:forEach var="oSbj"  items="${TenureStatusDetails.dList}" varStatus="indexx"> 
												...
												<c:choose>  
												    <c:when test="${oSbj.rr_tenure_status eq obj.rr_tenure_status }"> 
												      	<a onclick="deleteRow('${ oSbj.rr_tenure_status }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
    ...
    <div id="addUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/add-rr-tenure-status" id="addTenureStatusForm" name="addTenureStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h6 class="modal-header">Add Tenure Status <span class="right modal-action modal-close"><span
                    ...
                    <div class="col m8 s12">
                        <div class="row">
                            <div class="input-field col s12 m12">
                                <input id="rr_verificationby_text" type="text" name="rr_tenure_status" class="validate" onkeyup="doValidate(this.value)">
                                <label for="rr_verificationby_text"> Tenure Status</label>
                             ...
                             <div class="col s12 m6">
                                <div class="center-align m-1">
                                  <!--   <button
                                        ...
                                        <a href="<%=request.getContextPath()%>/rr-tenure-status"
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-rr-tenure-status" id=updateTenureStatusForm name="updateTenureStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h6 class="modal-header bg-m">Update  Tenure Status <span class="right modal-action modal-close"><span
                            class="material-icons">close</span></span></h6>
                <div class="row">
                    <div class="col m8 s12 offset-m2">
                       <div class="row no-mar">
                         <div class="input-field col s12 m12">
                                ...
                                <label for="value_new"> Tenure Status</label>
...
<%-- <jsp:include page="../layout/footer.jsp"></jsp:include> --%>
	<form name="getForm" id="getForm" method="post">
    	<input type="hidden" name="rr_tenure_status" id="rr_tenure_status" />
    </form>
    ...
    <script>
        $(document).ready(function () {
          ...
          /*   // adding table data into table start
            ...
            $('#rr_tenure_status_table tbody').append(table_text);
            // adding table data into table ends */

            var table = $('#rr_tenure_status_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
        ...
        function addVerificationBy(){
          	 if(validator.form()){ 
      			...
      			document.getElementById("addTenureStatusForm").submit();	
             }
         }
        function updateVerificationBy(){
          	 if(validator1.form()){ 
       			$(".page-loader").show();
       			$("#addUpdateModal").modal();
       			document.getElementById("updateTenureStatusForm").submit();	
           }
       }
         var validator =  $('#addTenureStatusForm').validate({
         	 rules: {
         		 "rr_tenure_status": {
   			 		  required: true
         		 }
         ...
         var validator1 = $('#updateTenureStatusForm').validate({
        	 rules: {
        		 	"value_new": {
    			 		  required: true
        			 }
      ...
      function updateRow(no) {
          var rr_tenure_status = $('#statusId'+no).val();
          $('#value_old').val($.trim(rr_tenure_status))
          $('#onlyUpdateModal').modal('open');
          $('#onlyUpdateModal #value_new').val($.trim(rr_tenure_status)).focus();
      }
        	...
        	swal({
    	            ...
    	            if (isConfirm) {
    	               // swal("Deleted!", "Record has been deleted", "success");
    	                $(".page-loader").show();
    	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-rr-tenure-status');
    	    	    	$('#getForm').submit();
    	           }else {
