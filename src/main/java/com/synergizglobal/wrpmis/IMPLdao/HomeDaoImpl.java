...
import com.synergizglobal.wrpmis.common.PermissionURLs;
import com.synergizglobal.wrpmis.common.TimeAgo;
...
public class HomeDaoImpl implements HomeDao {
	...
	public List<TableauDashboard> getDashboardsList(String dashboardType,String base, User uObj) throws Exception {
		...
		try {
			...
			String qry = "SELECT distinct tum.dashboard_id,tum.dashboard_name,dashboard_url,tum.priority,icon_path,mobile_view "
					+ "FROM dashboard tum "
	...
	private List<TableauDashboard> getTableauSubList(String parentId, String base, User uObj, Connection connection) throws Exception {
		...
		try {
			String qry = "SELECT distinct tum.dashboard_id,tum.dashboard_name,tum.dashboard_url,tum.priority,icon_path,mobile_view  "
					+ "FROM dashboard tum "
	...
	private List<TableauDashboard> getTableauSubListLevel2(String parentId, String base, User uObj, Connection connection) throws Exception {
		...
		try {
			String qry = "SELECT distinct tum.dashboard_id,tum.dashboard_name,tum.priority,icon_path  "
					+ "FROM dashboard tum "
					+ "WHERE parent_dashboard_id_sr_fk <> tum.dashboard_id and parent_dashboard_id_sr_fk = ? "
					+ "and tum.soft_delete_status_fk = ? ";
	...
	public boolean userLoginTimeout() throws Exception {
		...
		try {			
			...
			while(rs.next()) {
				User obj = new User();
				obj.setUser_login_details_id("user_login_id");
