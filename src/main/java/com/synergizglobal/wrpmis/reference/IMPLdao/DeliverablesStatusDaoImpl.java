...
import com.synergizglobal.wrpmis.reference.Idao.DeliverablesStatusDao;
import com.synergizglobal.wrpmis.reference.model.Safety;
import com.synergizglobal.wrpmis.reference.model.TrainingType;
@Repository
public class DeliverablesStatusDaoImpl implements DeliverablesStatusDao{
	@Autowired
	...
	public boolean addDeliverablesStatus(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO deliverables_status"
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select status from deliverables_status where status NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='deliverables_status' and s.name='dbo'\r\n" + 
	...
	public boolean updateDeliverablesStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table deliverables_status NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE deliverables_status SET status= :status_new WHERE status= :status_old " ;
			...
			String  enableQry =	"Alter Table deliverables_status CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteDeliverablesStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from deliverables_status WHERE status= :status; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
