...
import com.synergizglobal.wrpmis.common.TableauTrustedTicket;
...
public class OverviewDashboardController {
	...
	@RequestMapping(value = "/ajax/getContractorRoleWorks", method = {RequestMethod.GET,RequestMethod.POST},produces=MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public List<OverviewDashboard>getContractorRoleWorks(HttpSession session) throws Exception {
		List<OverviewDashboard> overviewDashboard = null;

		try {
			String user_name = (String) session.getAttribute("USER_NAME");
			overviewDashboard = overviewDashboardService.getContractorRoleWorks(user_name);
	...
	@ResponseBody
	public int getWorkDroneSurveyCount(String project_id) throws Exception {
		int cnt = 0;
		try {
			cnt = overviewDashboardService.getWorkDroneSurveyCount(project_id);
		} catch (SQLException e) {
	...
	@RequestMapping(value="/taDashboardByWork?project_id={project_id}",method= {RequestMethod.GET,RequestMethod.POST})
	public ModelAndView taDashboardByWork(@PathVariable("project_id") String project_id,HttpSession session,HttpServletRequest request) {
		ModelAndView model = new ModelAndView();
		try {
		    model.setViewName(PageConstants.taDashboard);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("taDashboardByWork : " + e.getMessage());
		}
		return model;
	}	
	
	@RequestMapping(value="/AIIBDisbursement/{project_id}",method= {RequestMethod.GET,RequestMethod.POST})
	public ModelAndView taDashboard(@PathVariable("project_id") String project_id,HttpSession session,HttpServletRequest request) {
		ModelAndView model = new ModelAndView();
		try {
	...
	@RequestMapping(value="/work-overview-dashboard/{project_id}",method= {RequestMethod.GET,RequestMethod.POST})
	public ModelAndView overviewDashboardByWork(@PathVariable("project_id") String project_id,HttpSession session,HttpServletRequest request) {
		ModelAndView model = new ModelAndView();
		try {
		    model.setViewName(PageConstants.overviewDashboard);
			model.addObject("project_id", project_id);
			model.addObject("dashboard_type", "Works");
	...
	@RequestMapping(value="/work-dashboard/{dashboard_id}/{project_id}",method= {RequestMethod.GET,RequestMethod.POST})
	public ModelAndView loadWorkDashboard(
	    ...
	    try {
	        model.setViewName(PageConstants.workSingleDashboard);

	        model.addObject("dashboard_id", dashboardId);
	        model.addObject("project_id", projectId);
			model.addObject("dashboard_type", "Works");	        

	        // get tableau URL from DB/service
	       // String url = overviewDashboardService.getDashboardURL(dashboardId, projectId);

	        //model.addObject("dashboard_url", url);
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
	...
	@RequestMapping(value="/work-overview-dashboard/{project_id}/{dashboardId}",method= {RequestMethod.GET,RequestMethod.POST})
	public ModelAndView overviewDashboardListByWork(@PathVariable("project_id") String project_id,@PathVariable("dashboardId") String dashboardId,HttpSession session) {
		ModelAndView model = new ModelAndView();
		try {
		    model.setViewName(PageConstants.overviewDashboard);
	...
	@RequestMapping(value="/archive-work-overview-dashboard/{project_id}",method= {RequestMethod.GET,RequestMethod.POST})
	public ModelAndView archiveOverviewDashboardByWork(@PathVariable("project_id") String project_id,HttpSession session,HttpServletRequest request) {
		ModelAndView model = new ModelAndView();
		try {
		    model.setViewName(PageConstants.archiveDashboards);
			model.addObject("project_id", project_id);
...
//			String project_id = dObj.getproject_id();
...
//				}
//				if(!StringUtils.isEmpty(project_id) && !StringUtils.isEmpty(params)) {
//					params = params + "&"+obj.getSource_field_name()+"="+project_id;
//				}else if(!StringUtils.isEmpty(project_id)){
//					params = obj.getSource_field_name()+"="+project_id;
//				}
//				String server_name = "MRVC";
//				if(dashboardUrl.contains(".com/")) {
//					server_name = "Syntrack";
