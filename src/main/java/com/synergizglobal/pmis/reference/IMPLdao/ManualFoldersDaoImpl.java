...
@Repository
public class ManualFoldersDaoImpl implements ManualFoldersDao{
	...
	public List<Safety> getManualFoldersList() throws Exception {
		...
		try {
			String qry ="select manual_folders from manual_folders ";
	...
	public boolean addManualFolders(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO manual_folders"
					+ "( manual_folders) VALUES (:manual_folders)";
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as `manual_folders`,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select `manual_folders` from manual_folders where `manual_folders` NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getManual_folders();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'manual_folders' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'manual_folders' and TABLE_SCHEMA = 'pmis' ";
	...
	public boolean updateManualFolders(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE manual_folders SET `manual_folders`= :value_new WHERE `manual_folders`= :value_old " ;
	...
	public boolean deleteManualFolders(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from manual_folders WHERE `manual_folders`= :manual_folders; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
