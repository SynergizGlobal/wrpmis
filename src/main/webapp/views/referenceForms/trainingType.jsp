                        ...
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="training_type_table" class="mdl-data-table">
                                    ...
                                    <tbody>
										<c:forEach var="obj" items="${trainingTypeDetails.dList1}" varStatus="indexs">
											<tr><td>
												<input type="hidden" id="training_typeId${indexs.count}" value="${obj.training_type }" />
												${obj.training_type }</td>
											<c:forEach var="tObj" items="${trainingTypeDetails.tablesList}" varStatus="index">
												<td><c:forEach var="cObj" items="${trainingTypeDetails.countList}" >
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    
													    		<c:choose>  
																    <c:when test="${cObj.training_type eq obj.training_type }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
											...
											<td class="last-column "><a onclick="updateRow(${indexs.count})" class="btn waves-effect waves-light bg-m t-c modal-trigger " href="#"> <i class="fa fa-pencil" ></i></a>
										 	<c:forEach var="oSbj"  items="${trainingTypeDetails.dList}" varStatus="indexx"> 
												 
												<c:choose>  
												    <c:when test="${oSbj.training_type eq obj.training_type }"> 
												      	<a onclick="deleteRow('${ oSbj.training_type }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
    ...
    <div id="addUpdateModal" class="modal">
                    ...
                    <div class="col m8 s12">
                        <div class="row">
                            <div class="input-field col s12 m12">
                                <input id="training_type_text" name="training_type" type="text" class="validate">
                        ...
                        <div class="row">
                            <div class="col s12 m6">
                               <div class="center-align m-1">
										<button type="button" onclick="addTrainingType();" style="width: 100%;" class="btn waves-effect waves-light bg-m">Add</button>
								</div>
                            </div>
                            <div class="col s12 m6">
                                <div class="center-align m-1">
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-training-type" id=updateTrainingTypeForm name="updateTrainingTypeForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header bg-m">Update Training Type <span class="right modal-action modal-close"><span
                            class="material-icons">close</span></span></h5>
                <div class="row">
                    <div class="col m2 hide-on-small"></div>
                    <div class="col m8 s12">
                       <div class="row no-mar">
                        ...
                        <div class="row">
                            <div class="col s12 m6">
                                <div class="center-align m-1">
                                    <button style="width: 100%;" onclick="updateTrainingType()"
...
<%--   <jsp:include page="../layout/footer.jsp"></jsp:include> --%>
    ...
    <script>
        $(document).ready(function () {
          ...
          /*   var arr = ['External', 'Internal'];
            var table_text = '';
            $.each(arr, function (i, val) {
                table_text = table_text + ' <tr><td>' + val + '</td>' + '<td class="last-column"> <a href="#" class="btn waves-effect waves-light bg-m t-c" ' +
                    '<i class="fa fa-pencil"></i></a><a href="#" class="btn waves-effect waves-light bg-s t-c "><i class="fa fa-trash"></i></a></td></tr>';
            });
            $('#training_type_table tbody').append(table_text); */
            // adding table data into table ends

            var table = $('#training_type_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
        ...
        function addTrainingType(){
           	 if(validator.form()){ 
       			...
       			document.getElementById("trainingTypeForm").submit();	
           	}
        }
        function updateTrainingType(){
        	 if(validator1.form()){ 
     			$(".page-loader").show();
     			$("#onlyUpdateModal").modal();
     			document.getElementById("updateTrainingTypeForm").submit();	
         }
     }
        var validator =	$('#trainingTypeForm').validate({
       	 rules: {
       		 "training_type": {
			 		  required: true
			 	  }
      ...
      var validator1 = $('#updateTrainingTypeForm').validate({
          	 rules: {
          		 	"value_new": {
      			 		  required: true
          			 }
        ...
        function updateRow(no) {
            var training_type = $('#training_typeId'+no).val();
            $('#value_old').val($.trim(training_type))
            $('#onlyUpdateModal').modal('open');
            $('#onlyUpdateModal #value_new').val($.trim(training_type)).focus();
        }
        
        function deleteRow(val){
        	$("#training_type").val(val);
        	showCancelMessage();
      	    }
          	...
          	swal({
      	            ...
      	            if (isConfirm) {
      	               // swal("Deleted!", "Record has been deleted", "success");
      	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-training-type');
      	    	    	$('#getForm').submit();
      	           }else {
