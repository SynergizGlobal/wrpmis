...
public class FOBFileTypeDaoImpl implements FOBFileTypeDao{
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='fob_file_type' and s.name='dbo'\r\n" + 
	...
	public boolean updateFOBFileType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table fob_file_type NOCHECK Constraint All";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table fob_file_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
