...
public class DeliverablesDaoImpl implements DeliverablesDao{
	...
	public List<Deliverables> getDeliverablesList(Deliverables obj) throws Exception {
		...
		try {
			String qry ="select deliverable_id,w.work_short_name,c.contract_short_name, d.project_id_fk,p.project_name,d.contract_id_fk,c.contract_name ,deliverable_type,"
	...
	public Deliverables getDeliverables(Deliverables obj) throws Exception {
		...
		try {
			String qry ="select deliverable_id,c.contract_short_name, d.project_id_fk,p.project_name,d.contract_id_fk,c.contract_name ,deliverable_type,"
					+"deliverable_description,d.status_fk,d.milestone_id,d.milestone_payment,"
					+ "(SELECT cm.milestone_name from contract_milestones cm WHERE cm.milestone_id = d.milestone_id AND cm.contract_id_fk = d.contract_id_fk group by cm.milestone_name) as milestone_name, "
	...
	public boolean addDeliverables(Deliverables obj) throws Exception {
		...
		try {
			...
			String existing_deliverable_id = checkDeliverableIdExistsOrNot(obj);
			
			String insertQry = "INSERT INTO deliverables"
					+ "(project_id_fk, contract_id_fk,milestone_id, deliverable_type, deliverable_description, status_fk,milestone_payment,created_date,created_by)"
					+ "VALUES"
					+ "(:project_id_fk, :contract_id_fk,:milestone_id, :deliverable_type, :deliverable_description,:status_fk,:milestone_payment,CURRENT_TIMESTAMP,:user_name)";
			
			String updateQry = "UPDATE deliverables SET "
					+ "project_id_fk = :project_id_fk,  contract_id_fk = :contract_id_fk,milestone_id = :milestone_id, "
			...
			if(!StringUtils.isEmpty(existing_deliverable_id)) {
				obj.setDeliverable_id(existing_deliverable_id);
			    ...
			    if(count > 0) {
			    	 String deliverable_id = String.valueOf(keyHolder.getKey().intValue());
				...
				if(!StringUtils.isEmpty(obj.getDeliverable_document_names()) && obj.getDeliverable_document_names().length > 0) {
					...
					for (int i=0; i<length; i++) {
						...
						if(!StringUtils.isEmpty(deliverableDocument.getDeliverable_document_name())) {
							...
							BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(deliverableDocument);	
	...
	private String getAutoGeneratedDeliverableDocumentId(Deliverables obj) throws Exception {
		...
		try {
			...
			if(!StringUtils.isEmpty(objsList) && objsList.size() > 0){
				qry = "SELECT MAX(CAST(SUBSTRING( deliverable_document_id ,"  
						+ "LEN(deliverable_document_id) -  CHARINDEX('-',REVERSE(deliverable_document_id)) + 2 ," 
						+ "LEN(deliverable_document_id)  ) as int))+1 "
						+ "as deliverable_document_id from deliverable_documents "
						+ "where deliverable_document_id like '"+contract_id_milestone_id+"%' ";
	...
	public List<Deliverables> getContractsListForDeliverablesForm(Deliverables obj) throws Exception {
		List<Deliverables> objsList = null;
		try {
			String qry ="select c.contract_id as contract_id_fk,c.contract_name,c.contract_short_name,c.project_id_fk "
					+ "from contract c "
	...
	public int getTotalRecords(Deliverables obj, String searchParameter) throws Exception {
		...
		try {
			...
			if(!StringUtils.isEmpty(searchParameter)) {
				qry = qry + " and ( d.project_id_fk like ? or p.project_name like ? or c.project_id_fk like ? or w.work_short_name like ? or d.contract_id_fk like ? "
	...
	public List<Deliverables> getDeliverablesList(Deliverables obj, int startIndex, int offset, String searchParameter)
		...
		try {
			...
			if(!StringUtils.isEmpty(searchParameter)) {
				qry = qry + " and ( d.project_id_fk like ? or p.project_name like ?  or d.contract_id_fk like ? "
						+ " or c.contract_short_name like ? or deliverable_type like ? or deliverable_description like ? or status_fk like ?)";
				arrSize++;
				arrSize++;
				arrSize++;
	...
	public List<Deliverables> getDeliverablesConractMilestonesList(Deliverables obj) throws Exception {
		...
		try {
			String qry ="select contract_milestones_id as contract_milestones_id_fk,"
					+ "cm.contract_id_fk,c.project_id_fk,"
					+ "c.contract_id,c.contract_short_name,c.contract_name,"
					+ "p.project_id,p.project_name,c.project_id_fk,"
	...
	public String[] uploadDeliverablesData(List<Deliverables> deliverablesList, Deliverables deliverableObj)
		...
		try {
			...
			for (Deliverables obj : deliverablesList) {
				...
				row++;sheet = 1;
				if(!StringUtils.isEmpty(obj.getDeliverable_id())) {
	...
	private String checkDeliverableIdExistsOrNot(Deliverables obj) throws Exception {
		...
		try {
			...
			String qry = "SELECT deliverable_id FROM deliverables "
					+ "WHERE contract_id_fk = ? AND milestone_id = ?";			
			
			stmt = connection.prepareStatement(qry);
			stmt.setString(1, obj.getContract_id_fk());
			stmt.setString(2, obj.getMilestone_id());
			
			rs = stmt.executeQuery();  
			if(rs.next()) {
				val = rs.getString("deliverable_id");
			}
		}catch(Exception e){ 
			throw new Exception(e);
		}finally {
