...
<%-- <jsp:include page="../layout/header.jsp"></jsp:include> --%>
                        ...
                        <div class="row">
                            ...
                            <div class="col m4 s12 center-align">
                                <a class="waves-effect waves-light btn bg-m modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add Issue Priority</a>
                            </div>
                            <div class="col m4 hide-on-small"></div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="Issue_Priority_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
										<c:forEach var="obj" items="${issuePriorityDetails.dList1}" varStatus="indexs">
											...
											<c:forEach var="tObj" items="${issuePriorityDetails.tablesList}" varStatus="index">
												...
												<td><c:forEach var="cObj" items="${issuePriorityDetails.countList}" >
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    
													    		<c:choose>  
																    <c:when test="${cObj.priority eq obj.priority }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
										 	...
										 	<c:forEach var="oSbj"  items="${issuePriorityDetails.dList}" varStatus="indexx"> 
												<c:choose>  
												    <c:when test="${oSbj.priority eq obj.priority }"> 
												      	<a onclick="deleteRow('${ oSbj.priority }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
    ...
    <div id="addUpdateModal" class="modal">
		<form action="<%=request.getContextPath() %>/add-issue-priority" id="addIssuePriorityForm" name="addIssuePriorityForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header">Add Issue Priority <span class="right modal-action modal-close"><span
    ...
    <script>
        $(document).ready(function () {
          ...
          /*   // adding table data into table start
            ...
            $('#Issue_Priority_table tbody').append(table_text);
            // adding table data into table ends */

            var table = $('#Issue_Priority_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
        ...
        var validator =  $('#addIssuePriorityForm').validate({
        	 rules: {
        		 "priority": {
  	        	 ...
  	        	 if(element.attr("id") == "Issue_Priority_text" ){
        ...
        var validator1 =  $('#updateIssuePriorityForm').validate({
       	 rules: {
       		 "value_new": {
 			 		  required: true
       		 }
        ...
        function updateRow(no) {
    	      var priority = $('#priorityId'+no).val();
    	      $('#value_old').val($.trim(priority))
    	      $('#onlyUpdateModal').modal('open');
    	      $('#onlyUpdateModal #value_new').val($.trim(priority)).focus();
    	  }
