...
import com.synergizglobal.wrpmis.Iservice.LandAcquisitionService;
...
import com.synergizglobal.wrpmis.model.LandAquisationPaginationObject;
...
public class LandAcquisitionController {
	...
	@Autowired
	LandAcquisitionProcessService landAcquisitionProcessService;
	...
	@RequestMapping(value = "/ajax/getTalukaVillageCollector", method = {RequestMethod.GET,RequestMethod.POST},produces=MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public List<LandAcquisition> getTalukaVillageCollector(@ModelAttribute LandAcquisition obj,HttpSession session) {
		List<LandAcquisition> TalukaVillageCollectorList = null;
		try {
			User uObj = (User) session.getAttribute("user");
			obj.setUser_type_fk(uObj.getUser_type_fk());
			obj.setUser_role_code(uObj.getUser_role_code());
			obj.setUser_id(uObj.getUser_id());
			TalukaVillageCollectorList = service.getTalukaVillageCollectorList(obj);
		}catch (Exception e) {
			e.printStackTrace();
			logger.error("TalukaVillageCollectorList : " + e.getMessage());
		}
		return TalukaVillageCollectorList;
	}	
	...
	public void exportLandAcquisition(HttpServletRequest request, HttpServletResponse response,HttpSession session,@ModelAttribute LandAcquisition dObj,RedirectAttributes attributes){
		...
		try {
			...
			if(dataList != null && dataList.size() > 0){
	            ...
	            String headerString = "Project ID^LA_ID^Survey Number^Type of Land^Sub Category of Land^Area^Area to be Acquired^Area Acquired^Land Status^Chainage From"
	...
	public void exportLand(HttpServletRequest request, HttpServletResponse response,HttpSession session,@ModelAttribute LandAcquisition dObj,RedirectAttributes attributes){
		...
		try {
			...
			XSSFRow headingInstrRow38 = Instruction.createRow(40);
	            ...
	            String headerString = "Project ID - Name^LA_ID^Survey Number^Type of Land^Sub Category of Land^Area^Area to be Acquired^Area Acquired^Land Status^Chainage From^Chainage To^Village^Taluka^Latitude^Longitude^Dy SLR^SDO^Collector^Proposal submission Date to collector^JM Fee Letter received Date^JM Fee Amount^JM Fee Paid Date^JM Start Date^JM Completion Date^JM Sheet Date to SDO^JM Remarks^JM Approval^Special Feature^Remarks^Issues";
	...
	@RequestMapping(value = "/land-acquisition-process", method = {RequestMethod.GET})
	public ModelAndView showLandAcquisitionPage(HttpSession session,@ModelAttribute LandAcquisition obj){
		ModelAndView model = new ModelAndView();
		try{
			model.setViewName(PageConstants.landAcquisitionProcess);
			model.addObject("action", "add");
	...
	@RequestMapping(value = "/land-acquisition-process/{la_id}", method = {RequestMethod.GET,RequestMethod.POST})
	public ModelAndView getLandAcquisitionFormProcess(@ModelAttribute LandAcquisitionProcess obj,@PathVariable("la_id") Long la_id  ,HttpSession session ){
		ModelAndView model = new ModelAndView();
		try{
			model.setViewName(PageConstants.landAcquisitionProcess);
			model.addObject("action", "add");
			...
			List<LandAcquisitionProcess> projectsList = service.getLandProcessProjectsList(obj);
			model.addObject("projectsList", projectsList);
			
			obj.setId(la_id);
			LandAcquisitionProcess LAProcessDetails = service.getLandAcquisitionProcessForm(obj);
			model.addObject("LADetails", LAProcessDetails);			
	...
	@RequestMapping(value = "/land-acquisition-process-grid",
	        method = {RequestMethod.GET, RequestMethod.POST})
	public ModelAndView showLandAcquisitionGridPage(
	        HttpSession session,
	        @ModelAttribute LandAcquisition obj) throws Exception {

	    ModelAndView model = new ModelAndView(PageConstants.landAcquisitionProcessGrid);

	    List<LandAcquisition> projectsList =
	            service.getLandProcessProjectsList(obj);
    ...
    @RequestMapping(value = "/add-land-acquisition-process", method = {RequestMethod.POST})
    public String addLandAcquisitionProcess(
            @ModelAttribute LandAcquisitionProcess landAcqProcess,
            @RequestParam(value = "privateLandsJson", required = false) String privateJson,
            @RequestParam(value = "governmentLandsJson", required = false) String govtJson,
            @RequestParam(value = "forestLandsJson", required = false) String forestJson,
        ...
        try {
            ...
            // ===== PRIVATE LANDS =====
            if (privateJson != null && !privateJson.isEmpty()) {
                PrivateLand[] privateLandsArray = mapper.readValue(privateJson, PrivateLand[].class);
                landAcqProcess.setPrivateLands(Arrays.asList(privateLandsArray));
            }

            // ===== GOVERNMENT LANDS =====
            if (govtJson != null && !govtJson.isEmpty()) {
                GovernmentLand[] governmentLandsArray = mapper.readValue(govtJson, GovernmentLand[].class);
                landAcqProcess.setGovernmentLands(Arrays.asList(governmentLandsArray));
            }

            // ===== FOREST LANDS =====
            if (forestJson != null && !forestJson.isEmpty()) {
                ForestLand[] forestLandsArray = mapper.readValue(forestJson, ForestLand[].class);
                landAcqProcess.setForestLands(Arrays.asList(forestLandsArray));
            }

            // ===== Debug logs =====
            System.out.println("PrivateLands size: " +
                    (landAcqProcess.getPrivateLands() == null ? 0 : landAcqProcess.getPrivateLands().size()));
            System.out.println("GovernmentLands size: " +
                    (landAcqProcess.getGovernmentLands() == null ? 0 : landAcqProcess.getGovernmentLands().size()));
            System.out.println("ForestLands size: " +
                    (landAcqProcess.getForestLands() == null ? 0 : landAcqProcess.getForestLands().size()));

            // ===== INSERT OR UPDATE =====
            if (landAcqProcess.getId() == null) {
                landAcquisitionProcessService.insertLandAcquisitionProcess(landAcqProcess);
            } else {
                landAcquisitionProcessService.updateLandAcquisitionProcess(landAcqProcess);
            }

            redirectAttributes.addFlashAttribute("msg", "Saved Successfully!");
            return "redirect:/land-acquisition-process-grid";
	...
	@RequestMapping(
	        value = "/ajax/get-land-acquisition-process",
	        method = { RequestMethod.GET, RequestMethod.POST }
	)
	public void getLandAcquisitionList(
	    ...
	    try {
	        // ===== DataTables params =====
	        int draw = request.getParameter("draw") != null
	                ? Integer.parseInt(request.getParameter("draw")) : 0;

	        int start = request.getParameter("start") != null
	                ? Integer.parseInt(request.getParameter("start")) : 0;

	        int length = request.getParameter("length") != null
	                ? Integer.parseInt(request.getParameter("length")) : 10;

	        // ===== Filters =====
	        String projectId  = request.getParameter("project_id");
	        String laFileType = request.getParameter("la_file_type");
	        String searchValue = request.getParameter("search[value]");

	        // ===== DB calls =====
	        List<LandAcquisitionProcess> data = landAcquisitionProcessService.getLandAcquisitionList(projectId, laFileType,searchValue, start, length);

	        long totalCount =landAcquisitionProcessService.getLandAcquisitionCount(projectId, laFileType,searchValue);

	        // ===== Required DataTables response =====
	        jsonResponse.put("draw", draw);
	        jsonResponse.put("recordsTotal", totalCount);
	        jsonResponse.put("recordsFiltered", totalCount);
	        jsonResponse.put("data", data == null ? new ArrayList<>() : data);

	    } catch (Exception e) {
	        e.printStackTrace();

	        // ===== VERY IMPORTANT =====
	        jsonResponse.put("draw", 0);
	        jsonResponse.put("recordsTotal", 0);
	        jsonResponse.put("recordsFiltered", 0);
	        jsonResponse.put("data", new ArrayList<>());
	        jsonResponse.put("error", "Server error while loading data");
	    }

	    response.getWriter().write(mapper.writeValueAsString(jsonResponse));
	}

	public int getTotalLARecords(LandAcquisition obj, String searchParameter,HttpSession session) {
		int totalRecords = 0;
		try {
			User uObj = (User) session.getAttribute("user");
			obj.setUser_type_fk(uObj.getUser_type_fk());
			obj.setUser_role_code(uObj.getUser_role_code());
			obj.setUser_id(uObj.getUser_id());
			totalRecords = service.getTotalLARecords(obj, searchParameter);
		} catch (Exception e) {
			logger.error("getTotalRecords : " + e.getMessage());
		}
		return totalRecords;
	...
	public List<LandAcquisition> createLAPaginationData(int startIndex, int offset, LandAcquisition obj, String searchParameter,HttpSession session) {
		List<LandAcquisition> earthWorkList = null;
		try {
			User uObj = (User) session.getAttribute("user");
			obj.setUser_type_fk(uObj.getUser_type_fk());
			obj.setUser_role_code(uObj.getUser_role_code());
			obj.setUser_id(uObj.getUser_id());
			earthWorkList = service.getLandAcquisitionProcessList(obj, startIndex, offset, searchParameter);
		} catch (Exception e) {
			logger.error("createPaginationData : " + e.getMessage());
		}
		return earthWorkList;
	...
	@RequestMapping(value = "/ajax/check-project-exists", method = RequestMethod.GET)
	@ResponseBody
	public boolean checkProjectExists(
	        @RequestParam("projectId") String projectId) throws Exception {

	    return landAcquisitionProcessService
	            .isProjectAlreadyExists(projectId);
	}
