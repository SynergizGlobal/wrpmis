	...
	public List<Contract> getProcurementStatus(Contract obj) throws Exception {
		...
		try {
			String qry = "select * from (select work_name ,sum(awarded_cost) as awarded_cost,sum(estimated_cost) as estimated_cost,sum(ifnull(total,0)) as total,sum(contract_details_types) as contract_details_types from (select work_name,hoddt.department_name,round(sum(ifnull(awarded_cost,0)*ifnull(awarded_cost_units,0))/10000000,2) as awarded_cost,\r\n"
					...
					+ "                    \r\n"
					+ "                    and contract_name not like '%Miscellaneous%' and  contract_name not like '%Expense - Land%') as total,\r\n"
					+ "                    \r\n"
					+ "       (select count(*)\r\n"
					+ " \r\n"
					+ " from contract c  \r\n"
					...
					+ "                    \r\n"
					+ "                    and contract_name not like '%Miscellaneous%' and  contract_name not like '%Expense - Land%'                    \r\n"
					+ "                    \r\n"
					...
					+ " UNION ALL \r\n"
					+ " \r\n"
					+ " select hoddt.department_name,round(sum(ifnull(awarded_cost,0)*ifnull(awarded_cost_units,0))/10000000,2) as awarded_cost,\r\n"
					+ " \r\n"
					+ " round(sum(ifnull(estimated_cost,0)*ifnull(estimated_cost_units,0))/10000000,2) as estimated_cost ,\r\n"
					+ " \r\n"
					+ " \r\n"
	...
	public List<Contract> getFinanceReportContracts(String work_id) throws Exception {
		...
		try {
			String qry ="select work_short_name as contract_name,'' as value,'' as actual_physical_progress,'' as actual_financial_progress from work where work_id='"+work_id+"'\r\n"
					...
					+ "where c.status='Open' and c.contract_name not like '%Miscellaneous%' and  c.contract_name not like '%Expense - Land%'\r\n"
