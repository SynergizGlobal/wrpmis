...
public class ProjectWorkOverviewReportDaoImpl implements ProjectWorkOverviewReportDao{
	...
	public List<Contract> getProcurementStatus(Contract obj) throws Exception {
		...
		NumberFormat numberFormatter = new DecimalFormat("#0.00");
		try {
			String qry = "select * from (select work_name,count(c.contract_id) as 'total',(select sum(ifnull(estimated_cost,0)*ifnull(estimated_cost_units,0)) as awarded_cost\r\n"
					+ "                    from contract c  \r\n"
					...
					+ "                    \r\n"
					+ "                     (select sum(ifnull(awarded_cost,0)*ifnull(awarded_cost_units,0)) as awarded_cost\r\n"
					+ "                    from contract c  \r\n"
					+ "					left join work w on c.work_id_fk = w.work_id COLLATE utf8mb4_unicode_ci  \r\n"
					+ "					left join contractor cr on c.contractor_id_fk = cr.contractor_id \r\n"
					+ "					left join project p on w.project_id_fk = p.project_id \r\n"
					...
					+ "left join contract c on c.contract_id=s.contract_id \r\n"
					+ "where c.contract_id is not null and work_id='"+obj.getWork_id_fk()+"' and contract_status_fk<>'Not Awarded'),'') as scope_of_contract\r\n"
					+ "\r\n"
					+ "                    \r\n"
					+ "                    from contract c  \r\n"
					+ "					left join work w on c.work_id_fk = w.work_id COLLATE utf8mb4_unicode_ci  \r\n"
