...
<head>
	<meta charset="UTF-8">
	<title>Progress Entry Report - Reports - PMIS</title>
					...
					<div class="center-align">
						<span class="card-title headbg">
							<div class="center-align p-2 bg-m m-b-2">
								<h6 id="rptName">Progress Entry Report</h6>
							</div>
						</span>
					</div>
					<!-- form start-->
					<form action="<%=request.getContextPath() %>/generate-progress-update-report" id="progressUpdateReportForm" name="progressUpdateReportForm" method="post" target="_blank">
						<div class="row">
						<div class="col s12 m12 l7 offset-l2 offset-m1">
							<div class="row no-mar" style="margin-bottom:0;">
								<div class="col s6 m3 l4 input-field"> 
									<p class="searchable_label">Project</p>
									<select class="searchable validate-dropdown" id="project_id" name="project_id" onchange="addInQueProject(this.value);getContractsList('');getContractorsList(''); getHodList('');getDyhodList('');resetFilterDropDowns();">
 ...
 /*      $(document).on('focus', '.datepicker',function(){
      ...
      let date_pickers = document.querySelectorAll('.datepicker');
        ...
        $(document).ready(function () {
            ...
            $('.searchable').select2();
            var filters = window.localStorage.getItem("progressReportFilters");
        ...
        function clearFilters(){
            ...
            window.localStorage.setItem("progressReportFilters",'');
        	window.location.href= "<%=request.getContextPath()%>/progress-update-report";
        }
        ...
        function getFobList(fob) {
           ...
           // if ($.trim(contract_id) != "" && $.trim(fob_id_fk) == "") {
            	...
            	var myParams = {project_id : project_id, work_id : work_id, contract_id : contract_id,fob_id_fk :fob_id_fk, contractor_id : contractor_id, hod : hod, dyhod : dyhod };
                $.ajax({
                    url: "<%=request.getContextPath()%>/ajax/getFobFilterListInProgressReport",
                    data: myParams, cache: false,async: false,
                    success: function (data) {
                        if (data.length > 0) {
                            $.each(data, function (i, val) {
        ...
        function getContractorsList(contractor) 
        {
            ...
            if ($.trim(contractor_id) == "") {
            	...
            	var myParams = {project_id : project_id, work_id : work_id, contract_id : contract_id,fob_id_fk : fob_id_fk, contractor_id : contractor_id, hod : hod, dyhod : dyhod };
                $.ajax({
	            	url: "<%=request.getContextPath()%>/ajax/getContractorsFilterListInProgressReport",
	                data: myParams, cache: false,async: false,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) 
        ...
        function getHodList(hodVal) {
            ...
            if ($.trim(hod) == "") {
            	...
            	var myParams = {project_id : project_id, work_id : work_id, contract_id : contract_id,fob_id_fk : fob_id_fk, contractor_id : contractor_id, hod : hod, dyhod : dyhod };
                $.ajax({
	            	url: "<%=request.getContextPath()%>/ajax/getHodFilterListInProgressReport",
	                data: myParams, cache: false,async: false,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) 
        ...
        function getDyhodList(dyHodVal){
            ...
            if ($.trim(dyhod) == "") {
            	...
            	var myParams = {project_id : project_id, work_id : work_id, contract_id : contract_id,fob_id_fk : fob_id_fk, contractor_id : contractor_id, hod : hod, dyhod : dyhod };
                $.ajax({
	            	url: "<%=request.getContextPath()%>/ajax/getDyhodFilterListInProgressReport",
	                data: myParams, cache: false,async: false,
	                success: function (data) {
	                    if (data.length > 0) {
	                        $.each(data, function (i, val) 
        ...
        var validator = $('#progressUpdateReportForm').validate({
