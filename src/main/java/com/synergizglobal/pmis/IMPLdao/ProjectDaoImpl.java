...
public class ProjectDaoImpl implements ProjectDao {
	...
	public boolean addProject(Project project)throws Exception{
		...
		try{
			...
			String maxIdQry = "SELECT top 1 CONCAT(SUBSTRING(project_id, 1, LEN(project_id)-2),SUBSTRING(cast(MAX(SUBSTRING(project_id, 2, LEN(project_id)))+1 as varchar),0,2) ) AS project_id FROM project  group by project_id order by project_id desc" ;
