...
@Repository
public class UserAccessTypeDaoImpl implements UserAccessTypeDao{
	...
	public boolean addUserAccessType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO user_access_type"
					+ "( user_access_type, user_access_table) VALUES (:user_access_type, :user_access_table)";
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as `user_access_type`,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select user_access_type, user_access_table from user_access_type where `user_access_type` NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'user_access_type' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'user_access_type' and TABLE_SCHEMA = 'pmis' ";
	...
	public boolean updateUserAccessType(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE user_access_type SET `user_access_type`= :value_new,user_access_table= :user_access_table_new WHERE `user_access_type`= :value_old " ;
	...
	public boolean deleteUserAccessType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from user_access_type WHERE `user_access_type`= :user_access_type_val; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
