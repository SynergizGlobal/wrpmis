...
import com.synergizglobal.wrpmis.reference.Idao.TrainingTypeDao;
...
public class TrainingTypeDaoImpl implements TrainingTypeDao{
	...
	public List<TrainingType> getTrainingTypesList() throws Exception {
		...
		try {
			String qry ="select training_type from training_type ";
	...
	public boolean addTrainingType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO training_type"
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as training_type,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select training_type from training_type where training_type NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getTraining_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='training_type' and s.name='dbo'\r\n" + 
	...
	public boolean updateTrainingType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table training_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE training_type SET training_type= :value_new WHERE training_type= :value_old " ;
			...
			String  enableQry =	"Alter Table training_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteTrainingType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from training_type WHERE training_type= :training_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
