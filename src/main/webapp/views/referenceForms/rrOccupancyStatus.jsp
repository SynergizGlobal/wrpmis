...
<head>
    ...
    <title>RR Occupancy Status</title>
...
<body>
                        ...
                        <div class="row no-mar">
                            <div class="col s12 center-align">
                                <a class="waves-effect waves-light btn bg-s modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add Occupancy Status</a>
                            </div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="rr_occupancy_status_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
											...
											<c:forEach var="tObj" items="${rrOccupancyStatus.tablesList}" varStatus="index">
												...
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    		...
													    		<c:choose>  
																    <c:when test="${cObj.rr_occupancy_status eq obj.rr_occupancy_status }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
										 	...
										 	<c:forEach var="oSbj"  items="${rrOccupancyStatus.dList}" varStatus="indexx"> 
												...
												<c:choose>  
												    <c:when test="${oSbj.rr_occupancy_status eq obj.rr_occupancy_status }"> 
												      	<a onclick="deleteRow('${ oSbj.rr_occupancy_status }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
    ...
    <div id="addUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/add-rr-occupancy-status" id="addOccupancyStatusForm" name="addOccupancyStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h6 class="modal-header">Add Occupancy Status <span class="right modal-action modal-close"><span
                             ...
                             <div class="col s12 m6">
                                <div class="center-align m-1">
                                  <!--   <button
                                        ...
                                        <a href="<%=request.getContextPath()%>/rr-occupancy-status"
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-rr-occupancy-status" id=updateOccupancyStatusForm name="updateOccupancyStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h6 class="modal-header bg-m">Update Occupancy Status <span class="right modal-action modal-close"><span
                            class="material-icons">close</span></span></h6>
                <div class="row">
                    <div class="col m8 s12 offset-m2">
                       <div class="row no-mar">
                         <div class="input-field col s12 m12">
                                ...
                                <label for="value_new"> Occupancy Status</label>
...
<%-- <jsp:include page="../layout/footer.jsp"></jsp:include> --%>
	<form name="getForm" id="getForm" method="post">
    	<input type="hidden" name="rr_occupancy_status" id="rr_occupancy_status" />
    </form>
    ...
    <script>
        $(document).ready(function () {
          ...
          /*   // adding table data into table start
            ...
            $('#rr_occupancy_status_table tbody').append(table_text);
            // adding table data into table ends */

            var table = $('#rr_occupancy_status_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
        ...
        function updateOccupancyStatus(){
          	 if(validator1.form()){ 
       			$(".page-loader").show();
       			$("#addUpdateModal").modal();
       			document.getElementById("updateOccupancyStatusForm").submit();	
           }
       }
         var validator =  $('#addOccupancyStatusForm').validate({
         	 rules: {
         		 "rr_occupancy_status": {
   			 		  required: true
         		 }
         ...
         var validator1 = $('#updateOccupancyStatusForm').validate({
        	 rules: {
        		 	"value_new": {
    			 		  required: true
        			 }
      ...
      function updateRow(no) {
          var rr_occupancy_status = $('#statusId'+no).val();
          $('#value_old').val($.trim(rr_occupancy_status))
          $('#onlyUpdateModal').modal('open');
          $('#onlyUpdateModal #value_new').val($.trim(rr_occupancy_status)).focus();
      }
        	...
        	swal({
    	            ...
    	            if (isConfirm) {
    	               // swal("Deleted!", "Record has been deleted", "success");
    	                $(".page-loader").show();
    	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-rr-occupancy-status');
    	    	    	$('#getForm').submit();
    	           }else {
