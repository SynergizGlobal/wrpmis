...
<head>
    ...
    <title>Data Gathering Status</title>
...
<body>
                        ...
                        <div class="row no-mar">
                            <div class="col s12 center-align">
                                <a class="waves-effect waves-light btn bg-s modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add Data Gathering Status</a>
                            </div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="data_gathering_status_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                            <th>Data Gathering Status</th>
                                            <c:forEach var="tObj" items="${dataGatheringStatusDetails.tablesList}" >
                                    ...
                                    <tbody>
										<c:forEach var="obj" items="${dataGatheringStatusDetails.dList1}" varStatus="indexs">
											<tr><td>
												<input type="hidden" id="statusId${indexs.count}" value="${obj.status }"  class="findLengths"/>
												${obj.status }</td>
											<c:forEach var="tObj" items="${dataGatheringStatusDetails.tablesList}" varStatus="index">
											 
												<td><c:forEach var="cObj" items="${dataGatheringStatusDetails.countList}" >
												<c:choose> 
											...
											<td class="last-column "><a onclick="updateRow(${indexs.count})" class="btn waves-effect waves-light bg-m t-c"> <i class="fa fa-pencil" ></i></a>
										 	<c:forEach var="oSbj"  items="${dataGatheringStatusDetails.dList}" varStatus="indexx"> 
    ...
    <div id="addUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/add-data-gathering-status" id="addStatusForm" name="addStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header">Add Data Gathering Status <span class="right modal-action modal-close"><span
                    ...
                    <div class="col m8 s12">
                        <div class="row">
                            <div class="input-field col s12 m12">
                                <input id="data_gathering_status_text" type="text" name="status" class="validate" onkeyup="doValidate(this.value)">
                                <label for="data_gathering_status_text">Data Gathering Status</label>
                             ...
                             <div class="col s6 m6">
                                <div class="center-align m-1">
                                  <!--   <button
                                        ...
                                        <a href="<%=request.getContextPath()%>/data-gathering-status"
    ...
    <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-data-gathering-status" id=updateStatusForm name="updateStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header bg-m">Update Data Gathering Status <span class="right modal-action modal-close" onclick="removeErrorMsg()"><span
                            class="material-icons">close</span></span></h5>
                <div class="row">
                    <div class="col m2 hide-on-small"></div>
                    <div class="col m8 s12">
                       <div class="row no-mar">
                         <div class="input-field col s12 m12">
                                ...
                                <label for="status_new">Data Gathering Status</label>
...
<script src="/pmis/resources/js/dataTables.fixedColumns.min.js"></script>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table = $('#data_gathering_status_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
       ...
       var validator = $('#addStatusForm').validate({
       	 rules: {
   	        	 ...
   	        	 if(element.attr("id") == "data_gathering_status_text" ){
         	...
         	swal({
    	            ...
    	            if (isConfirm) {
    	               // swal("Deleted!", "Record has been deleted", "success");
    	                $(".page-loader").show();
    	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-data-gathering-status');
    	    	    	$('#getForm').submit();
    	           }else {
