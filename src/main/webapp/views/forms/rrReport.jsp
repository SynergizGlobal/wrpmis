                    ...
                    <div class="center-align m-1 close-message">${error}</div>
			            <form action="<%=request.getContextPath() %>/generate-rr-report" id="RRReportForm" name="RRReportForm" method="post" target="_blank">	                              
                                ...
                                <input id="work_name" name="work_short_name" type="hidden"/>
                                    ...
                                    <select class="searchable validate-dropdown" id="work_id" name="work_id" onchange="addInQueWork(this.value);getRRReport();">
                                        <option value="">Select </option>
                                          <c:forEach var="obj" items="${worksList }">
                                      	   <option value= "${obj.work_id}">${obj.work_id} - ${obj.work_short_name}</option>
                                         </c:forEach>
                                    </select>
                                    <span id="sub_workError" class="error-msg" ></span>
                                </div>
    ...
    function getErrorMessage(jqXHR, exception) {
        ...
        function getWorksList(workVal) {
            ...
            if ($.trim(work_id) == "") { 
                ...
                var myParams = { type_of_use: type_of_use, work_id : work_id,location_name : location_name,project_id_fk : project_id_fk };
                $.ajax({
                    url: "<%=request.getContextPath()%>/ajax/getWorksListForRRReportForm",
                    data: myParams, cache: false,async: false,
                    success: function (data) {
                        if (data.length > 0) {
                            $.each(data, function (i, val) {
                                var selectedFlag = (workVal == val.work_id)?'selected':'';
                                $("#work_id").append('<option value="' + val.work_id + '"'+selectedFlag+'>' + $.trim(val.work_id) + " - "+$.trim(val.work_short_name) + '</option>');
