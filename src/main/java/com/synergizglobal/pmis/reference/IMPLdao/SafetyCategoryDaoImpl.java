...
@Repository
public class SafetyCategoryDaoImpl implements SafetyCategoryDao{
	...
	public boolean addSafetyCategory(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO safety_category"
					+ "( category, short_description) VALUES (:category, :short_description)";
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select category, short_description from safety_category where category NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select t1.name as tname,(select c2.name from sys.columns c2 where c2.object_id=t1.object_id\r\n" + 
					...
					"WHERE t.name = 'safety_category' and s.name = 'dbo'),'%')) as COLUMN_NAME,'' as CONSTRAINT_NAME,\r\n" + 
					"'safety_category' as REFERENCED_TABLE_NAME,\r\n" + 
					...
					"WHERE t2.name = 'safety_category' and s2.name = 'dbo')) as REFERENCED_COLUMN_NAME\r\n" + 
					...
					"left join sys.tables t on t.object_id = c.object_id\r\n" + 
					"left join sys.schemas s on s.schema_id = t.schema_id\r\n" + 
					"WHERE t.name = 'safety_category' and s.name = 'dbo'),'%'))\r\n" + 
					"and t1.name !=  'safety_category'";
	...
	public boolean updateSafetyCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE safety_category SET category= :value_new,short_description= :short_description_new WHERE category= :value_old " ;
	...
	public boolean deleteSafetyCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from safety_category WHERE category= :category; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
