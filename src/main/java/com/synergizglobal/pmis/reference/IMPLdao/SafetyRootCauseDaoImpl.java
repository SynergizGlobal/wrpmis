...
@Repository
public class SafetyRootCauseDaoImpl implements SafetyRootCauseDao{
	...
	public boolean addSafetyRootCause(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO safety_root_cause"
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as root_cause,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select root_cause from safety_root_cause where root_cause NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getRoot_cause();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select t1.name as tname,(select c2.name from sys.columns c2 where c2.object_id=t1.object_id\r\n" + 
					...
					"WHERE t.name = 'safety_root_cause' and s.name = 'dbo'),'%')) as COLUMN_NAME,'' as CONSTRAINT_NAME,\r\n" + 
					"'safety_root_cause' as REFERENCED_TABLE_NAME,\r\n" + 
					...
					"WHERE t2.name = 'safety_root_cause' and s2.name = 'dbo')) as REFERENCED_COLUMN_NAME\r\n" + 
					...
					"left join sys.tables t on t.object_id = c.object_id\r\n" + 
					"left join sys.schemas s on s.schema_id = t.schema_id\r\n" + 
					"WHERE t.name = 'safety_root_cause' and s.name = 'dbo'),'%'))\r\n" + 
					"and t1.name !=  'safety_root_cause'";
	...
	public boolean updateSafetyRootCause(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE safety_root_cause SET root_cause= :value_new WHERE root_cause= :value_old " ;
	...
	public boolean deleteSafetyRootCause(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from safety_root_cause WHERE root_cause= :root_cause; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
