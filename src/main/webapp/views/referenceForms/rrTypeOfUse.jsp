...
<body>
                        ...
                        <div class="row no-mar">
                            <div class="col s12 center-align">
                                <a class="waves-effect waves-light btn bg-s modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add  Type Of Use</a>
                            </div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="rr_type_of_use_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                            <th> Type Of Use</th>
											...
											<c:forEach var="tObj" items="${rrSVerificationByDetails.tablesList}" varStatus="index">
												...
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    		...
													    		<c:choose>  
																    <c:when test="${cObj.rr_type_of_use eq obj.rr_type_of_use }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
										 	...
										 	<c:forEach var="oSbj"  items="${rrSVerificationByDetails.dList}" varStatus="indexx"> 
												...
												<c:choose>  
												    <c:when test="${oSbj.rr_type_of_use eq obj.rr_type_of_use }"> 
												      	<a onclick="deleteRow('${ oSbj.rr_type_of_use }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
    ...
    <div id="addUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/add-rr-type-of-use" id="addTypeOfUseForm" name="addTypeOfUseForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h6 class="modal-header">Add Type Of Use <span class="right modal-action modal-close"><span
                             ...
                             <div class="col s12 m6">
                                <div class="center-align m-1">
                                  <!--   <button
                                        ...
                                        <a href="<%=request.getContextPath()%>/rr-type-of-use"
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-rr-type-of-use" id=updateTypeOfUseForm name="updateTypeOfUseForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h6 class="modal-header bg-m">Update  Type Of Use <span class="right modal-action modal-close"><span
                            class="material-icons">close</span></span></h6>
                <div class="row">
                    <div class="col m8 s12 offset-m2">
                       <div class="row no-mar">
                         <div class="input-field col s12 m12">
                                ...
                                <label for="value_new"> Type Of Use</label>
...
<%-- <jsp:include page="../layout/footer.jsp"></jsp:include> --%>
	<form name="getForm" id="getForm" method="post">
    	<input type="hidden" name="rr_type_of_use" id="rr_type_of_use" />
    </form>
    ...
    <script>
        $(document).ready(function () {
          ...
          /*   // adding table data into table start
            ...
            $('#rr_type_of_use_table tbody').append(table_text);
            // adding table data into table ends */

            var table = $('#rr_type_of_use_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
        ...
        function updateVerificationBy(){
          	 if(validator1.form()){ 
       			$(".page-loader").show();
       			$("#addUpdateModal").modal();
       			document.getElementById("updateTypeOfUseForm").submit();	
           }
       }
         var validator =  $('#addTypeOfUseForm').validate({
         	 rules: {
         		 "rr_type_of_use": {
   			 		  required: true
         		 }
         ...
         var validator1 = $('#updateTypeOfUseForm').validate({
        	 rules: {
        		 	"value_new": {
    			 		  required: true
        			 }
      ...
      function updateRow(no) {
          var rr_type_of_use = $('#statusId'+no).val();
          $('#value_old').val($.trim(rr_type_of_use))
          $('#onlyUpdateModal').modal('open');
          $('#onlyUpdateModal #value_new').val($.trim(rr_type_of_use)).focus();
      }
        	...
        	swal({
    	            ...
    	            if (isConfirm) {
    	               // swal("Deleted!", "Record has been deleted", "success");
    	                $(".page-loader").show();
    	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-rr-type-of-use');
    	    	    	$('#getForm').submit();
    	           }else {
