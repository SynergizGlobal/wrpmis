...
import com.synergizglobal.wrpmis.Idao.FortnightPlanDao;
...
import com.synergizglobal.wrpmis.model.FortnightPlan;
...
public class FortnightPlanDaoImpl implements FortnightPlanDao {
	...
	public List<FortnightPlan> getFortnightPlanList(FortnightPlan obj) throws Exception {
		...
		/*
		 * try { String qry =
		 * "SELECT distinct min(F.ID) as fortnightly_plan_id,category,contract_short_name,structure_type as structure_type_fk,structure, "
		 * +
		 ...
		 * "LEFT JOIN work w on c.work_id_fk =w.work_id  " + "where f.status='Active' "
		 * ; int arrSize = 0;
		 * 
		 ...
		 * 
		 * if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getContract_id_fk())) { qry = qry +
		 * " and f.contract_id_fk = ? "; arrSize++; } if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getWork_id_fk())) { qry = qry +
		 * " and c.work_id_fk = ? "; arrSize++; } if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getCategory())) { qry = qry + " and category = ? ";
		 * arrSize++; } if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getCritical_item())) { qry = qry +
		 * " and critical_item = ? "; arrSize++; }
		 * 
		 * if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getPeriod())) {
		 * String Str[]=obj.getPeriod().split("_"); if(Str.length==2) { qry = qry +
		 * " and fortnight_start = ? and fortnight_finish=? "; arrSize++; arrSize++; }
		 * else { qry = qry +
		 * " and (cast(fortnight_start as varchar) = ? or cast(fortnight_finish as varchar)=?) "
		 * ; arrSize++; arrSize++; }
		 * 
		 * }
		 * 
		 ...
		 * 
		 * if(obj.getCritical().compareTo("Yes")==0) { qry = qry +
		 * "  and isnull([float],0)<=15 "; } else
		 * if(obj.getCritical().compareTo("No")==0) { qry = qry +
		 * "  and isnull([float],0)>15 "; } }
		 * 
		 * qry = qry +
		 * " group by category,contract_short_name,structure_type,structure,expected_finish,[float]  "
		 * ;
		 * 
		 ...
		 * 
		 * if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getContract_id_fk())) { pValues[i++] =
		 * obj.getContract_id_fk(); } if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getWork_id_fk())) { pValues[i++] =
		 * obj.getWork_id_fk(); } if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getCategory())) { pValues[i++] = obj.getCategory();
		 * } if(!StringUtils.isEmpty(obj) &&
		 * !StringUtils.isEmpty(obj.getCritical_item())) { pValues[i++] =
		 * obj.getCritical_item(); }
		 * 
		 * if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getPeriod())) {
		 * String Str[]=obj.getPeriod().split("_"); if(Str.length==2) { pValues[i++] =
		 * Str[0]; pValues[i++] = Str[1]; } else { pValues[i++] = obj.getPeriod();
		 * pValues[i++] = obj.getPeriod(); } }
		 * 
		 * if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getCritical())) {
		 * //pValues[i++] = obj.getCritical(); }
		 * 
		 * objsList = jdbcTemplate.query( qry, pValues, new
		 * BeanPropertyRowMapper<FortnightPlan>(FortnightPlan.class)); }catch(Exception
	...
	public List<FortnightPlan> getFortnightPlanCategoryList() throws Exception {
		...
		try {
			...
			String qry = "SELECT distinct category  " + "from fortnight_temp f "
					+ "LEFT join contract c ON c.contract_id  = f.contract_id_fk "
	...
	public int getTotalRecords(FortnightPlan obj, String searchParameter) throws Exception {
		int totalRecords = 0;
		try {
			String qry = "SELECT count(*) as total_records " + "from fortnight_temp f "
					+ "LEFT JOIN contract c ON c.contract_id  = f.contract_id_fk "
					+ "LEFT JOIN work w on c.work_id_fk =w.work_id " + "where f.status='Active' ";
			...
			if (!StringUtils.isEmpty(searchParameter)) {
				qry = qry
						+ " and (c.contract_id like ? or c.contract_short_name like ? or category like ? or critical_item like ? "
	...
	@Override
	public List<FortnightPlan> getFortnightPlanList(FortnightPlan obj, int startIndex, int offset,
			String searchParameter) throws Exception {
		List<FortnightPlan> objsList = null;
		try {
			String qry = "SELECT *  " + "from fortnight_temp f "
					+ "LEFT JOIN contract c ON c.contract_id  = f.contract_id_fk " + "where f.status='Active'";
	...
	public List<FortnightPlan> getFortnightPlanWorkList() throws Exception {
		...
		try {
			String qry = "select work_id as work_id_fk,work_name,work_short_name " + "from work w "
					+ "LEFT OUTER JOIN project p ON project_id_fk = project_id " + "where work_id is not null ";
	...
	public List<FortnightPlan> getFortnightPlanContractList(FortnightPlan obj) throws Exception {
		...
		try {
			String qry = "select distinct c.contract_id as contract_id_fk,c.hod_user_id_fk,c.contract_name,c.contract_short_name,c.work_id_fk "
					+ "from contract c " + "left join contract_executive c1 on c1.contract_id_fk = c.contract_id "
					+ "LEFT JOIN [user] u ON c.hod_user_id_fk= u.user_id " + "where c.contract_id is not null ";
	...
	public List<FortnightPlan> getFortnightPlanPeriodList() throws Exception {
		...
		try {
			...
			String qry = "SELECT FORMAT (fortnight_start, 'dd MMMM, yy')+'-'+FORMAT (fortnight_finish, 'dd MMMM, yy') as period,cast(fortnight_start as varchar)+'_'+cast(fortnight_finish as varchar) as period_value "
					+ "from fortnight_temp f " + "LEFT join contract c ON c.contract_id  = f.contract_id_fk "
					+ "LEFT JOIN work w on c.work_id_fk =w.work_id " + "where f.status='Active' ";
	...
	public List<FortnightPlan> getContractListFilter(FortnightPlan obj) throws Exception {
		...
		try {
			String qry = "SELECT f.contract_id_fk,c.contract_short_name  " + "from fortnightly_plan f "
					+ "LEFT join contract c ON c.contract_id  = f.contract_id_fk "
					+ "LEFT JOIN work w on c.work_id_fk =w.work_id "
	...
	public List<FortnightPlan> getFortnightPlanManual(FortnightPlan obj) throws Exception {
		...
		try {
			...
			String qry = "SELECT distinct fortnightly_plan_update_data_id as fortnightly_plan_id,w.work_id as work_id_fk,critical_item as total_items,u.category,'' as contract_short_name,structure,'' as structure_type_fk, "
					...
					+ "plan_for_the_current_fortnight as planned_current_st,activity as activity_name,scope_of_work as scope,completion_status,u.remarks,data_id,period  from fortnightly_plan_update_data u LEFT JOIN work w on u.work_id =w.work_id where u.data_id = "
					+ obj.getFortnightly_plan_id();
			objsList = jdbcTemplate.query(qry, new BeanPropertyRowMapper<FortnightPlan>(FortnightPlan.class));
	...
	public List<FortnightPlan> getFortnightQuarterlyPlanItemList() throws Exception {
		List<FortnightPlan> objsList = new ArrayList<FortnightPlan>();
		try {
			String qry = "select distinct item " + "from fortnight_quarterly_plan where item is not null ";
	...
	public List<FortnightPlan> getFortnightQuarterlyPlanPeriodList() throws Exception {
		List<FortnightPlan> objsList = new ArrayList<FortnightPlan>();
		try {
			String qry = "select distinct period " + "from fortnight_quarterly_plan ";
	...
	public boolean insertQuarterlyPlan(FortnightPlan obj) throws Exception {
		...
		try {
			...
			if (!StringUtils.isEmpty(obj) && StringUtils.isEmpty(obj.getFortnightly_plan_id())) {
				...
				if (preparedStmtChild != null) {
					preparedStmtChild.close();
				}
				...
				if (preparedStmtRevisions != null) {
					preparedStmtRevisions.close();
				}
				...
				if (deleteChild != null) {
					preparedStmtDeleteRevisions.close();
				}
			 ...
			 * formHistory.setForm_details("Fortnight Quarterly Plan  "+obj.
			 * getFortnight_quarterly_plan_id() + " Updated");
			 * formHistory.setWork_id_fk(obj.getWork_id_fk());
			 * formHistory.setContract_id_fk(obj.getContract_id_fk());
			 * 
			 * flag = formsHistoryDao.saveFormHistory(formHistory);
	...
	public List<FortnightPlan> getItemListQuarterlyFilter(FortnightPlan obj) throws Exception {
		...
		try {
			...
			String qry = "SELECT distinct item  " + "from fortnight_quarterly_plan f "
					+ "LEFT JOIN work w on f.work_id_fk =w.work_id " + "where item is not null ";
	...
	public boolean updateQuarterlyPlanActivities(FortnightPlan obj) throws Exception {
		...
		try {
			 ...
			 * formHistory.setForm_action_type("Update");
			 * formHistory.setForm_details("Quarterly Fortnight Plan  "+obj.
			 * getFortnight_quarterly_plan_activity_id() + " Updated");
			 * formHistory.setWork_id_fk(obj.getWork_id_fk());
			 * formHistory.setContract_id_fk(obj.getContract_id_fk());
			 * 
			 * flag = formsHistoryDao.saveFormHistory(formHistory);
	...
	public List<FortnightPlan> getQuarterlyPlanManual(FortnightPlan obj) throws Exception {
		...
		try {
			...
			String qry = "SELECT p.fortnight_quarterly_plan_id as fortnightly_plan_id,fortnight as fortnight_date,work_id_fk,period,structure,units as unit,cumulative_progress as cum_progress,activity_name,item,criticality,scope_of_work as scope_of_work_quarterly,TDC as tdc_calendar,isnull(pending_progress,'') as pending_progress,backlog_from_previous_months,isnull(reason_for_shortfall,'')  as reason_for_shortfall "
					...
					+ "left join fortnight_quarterly_plan_activities a on a.fortnight_quarterly_plan_id=p.fortnight_quarterly_plan_id "
					+ "LEFT JOIN work w on p.work_id_fk =w.work_id "
					+ "where p.work_id_fk is not null and fortnight is not null and isnull(pending_progress,'')!='Completed'  and p.fortnight_quarterly_plan_id="
					+ obj.getFortnightly_plan_id();
	...
	public List<FortnightPlan> getTDCRevisions(FortnightPlan obj) throws Exception {
		...
		try {
			...
			String qry = "select fortnight_quarterly_plan_id,revision_no,tdc_date from fortnight_quarterly_plan_tdc_revisions where fortnight_quarterly_plan_id="
					+ obj.getFortnightly_plan_id();
			objsList = jdbcTemplate.query(qry, new BeanPropertyRowMapper<FortnightPlan>(FortnightPlan.class));
	...
	public List<FortnightPlan> getFortnightPlanContractList() throws Exception {
		List<FortnightPlan> objsList = new ArrayList<FortnightPlan>();
		try {
			String qry = "select contract_id as contract_id_fk,contract_name,contract_short_name " + "from project p "
					+ "LEFT OUTER JOIN work w ON w.project_id_fk = p.project_id "
					+ "LEFT OUTER JOIN contract c ON c.work_id_fk = w.work_id " + "where work_id is not null ";
	...
	public FortnightPlan generateFortnightReport(FortnightPlan obj) throws Exception {
		...
		try {
			...
			objsList = (FortnightPlan) jdbcTemplate.query(qry, pValues,
					new BeanPropertyRowMapper<FortnightPlan>(FortnightPlan.class));
	...
	public List<FortnightPlan> contractList(FortnightPlan obj) throws Exception {
		...
		try {
			String qry = "select distinct w.work_name,w.work_short_name, d1.department_name as department_name,w.project_id_fk,p.project_name,"
					...
					+ "c.hod_user_id_fk,c.dy_hod_user_id_fk," + "FORMAT(modified_date,'dd-MM-yyyy') as modified_date "
					+ "from contract c " + "left join work w on c.work_id_fk = w.work_id  "
	...
	private int getRowCheck(String date, String contract_short_name, String structure_type_fk, String structure,
		...
		try {
			...
			rowCount = (int) jdbcTemplate.queryForObject(qry,
					new Object[] { date, contract_short_name, structure_type_fk, structure, component }, int.class);
		} catch (Exception e) {
			throw new Exception(e);
		}
		return rowCount;
	...
	public int insertQuarterlyPlans(List<FortnightPlan> fortnightPlansList) throws Exception {
		...
		String sArray[] = null;
		sArray = new String[] { "1st January," + substringYear + "  - 31st March," + substringYear + "",
				"1st April," + substringYear + "  - 30th June," + substringYear + "",
				"1st July," + substringYear + "  - 30th September," + substringYear + "",
				"1st October," + substringYear + "  - 31st December," + substringYear + "" };
		...
		try {
			...
			for (FortnightPlan obj : fortnightPlansList) {
				if (!StringUtils.isEmpty(obj) && StringUtils.isEmpty(obj.getFortnightly_plan_id())) {
					...
					if (!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getFortnight_date())
							&& !StringUtils.isEmpty(obj.getActivity_name())) {
