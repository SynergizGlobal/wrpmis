...
/****************************************************************************/
	...
	public static void createTableForSafetyReport(WordprocessingMLPackage wordMLPackage, MainDocumentPart t,
		...
		RPr calibriNotBoldRPr = getRPr(factory, "Calibri", "000000", "22", STHint.EAST_ASIA,
				false, false, false, false);
		...
		List<String> openWorkIdsList = new ArrayList<>();
		List<String> closedWorkIdsList = new ArrayList<>();
		
		List<String> openProjectIDList = new ArrayList<>();
		List<String> openProjectNameList = new ArrayList<>();
		
		List<String> closedProjectIDList = new ArrayList<>();
		List<String> closedProjectNameList = new ArrayList<>();		
		
		List<String> openWorkNameList = new ArrayList<>();
		List<String> closedWorkNameList = new ArrayList<>();
		
		List<String> pushworkids = new ArrayList<>();
		List<String> pushcworkids = new ArrayList<>();
		
		if(!StringUtils.isEmpty(safetyData)) 
		{
			for (Safety pObj : safetyData) 
			{
				if(pObj.getStatus_fk().compareTo("Open")==0)
				{
				
					openProjectIDList.add(pObj.getProject_id_fk());
					openProjectNameList.add(pObj.getProject_name());					
					
				}
				if(pObj.getStatus_fk().compareTo("Closed")==0)
				{
					
					closedProjectIDList.add(pObj.getProject_id_fk());
					closedProjectNameList.add(pObj.getProject_name());					
		...
		for (int i2 = 0; i2 < lList.size(); i2++) 
		{
			...
			if(i2==0)
			{
				...
				for (int i3 = 0; i3 < openWorkIdsList.size(); i3++) 
				{
					...
					mergeCellsHorizontal(tableHead, 0, 0, 12);
					rowNum++;
				...
				int kl=i3+1;
				Tr hodRowHeader4 = factory.createTr();
				addTableCell(factory, wordMLPackage, hodRowHeader4, "1."+ kl+".Work Name:"+openWorkIdsList.get(i3)+" - "+openWorkNameList.get(i3), calibriNotBoldRPr, JcEnumeration.LEFT, true,"ffffff");
				...
				if(openWorkIdsList.size()>0)
				{
					if(!StringUtils.isEmpty(safetyData)) 
					{
						...
						for (Safety pObj : safetyData) 
						{
							...
							if(lList.get(i2).compareTo(pObj.getStatus_fk())==0  && pushworkids.indexOf(pObj.getContract_id_fk())==-1)
							{
				...
				if(!StringUtils.isEmpty(safetyData)) 
				{
					...
					for (Safety pObj : safetyData) 
					{
						...
						if(lList.get(i2).compareTo(pObj.getStatus_fk())==0  && pushworkids.indexOf(pObj.getContract_id_fk())==-1)
						{
							...
							addTableCell(factory, wordMLPackage, contentRow, String.valueOf(sNo++),
									garamondRPr, JcEnumeration.CENTER, hasBgColor, backgroundColor);
							addTableCell(factory, wordMLPackage, contentRow, pObj.getContract_id_fk()+"/ \n "+pObj.getContract_short_name(),
									garamondRPr, JcEnumeration.LEFT, hasBgColor, backgroundColor);
							addTableCell(factory, wordMLPackage, contentRow, pObj.getContractor_name(),
									garamondRPr, JcEnumeration.LEFT, hasBgColor, backgroundColor);	
							addTableCell(factory, wordMLPackage, contentRow, pObj.getDesignation(),
			...
			{
				for (int i3 = 0; i3 < closedWorkIdsList.size(); i3++) 
				{
					
					Tbl tableHead = factory.createTbl();
					setLandscapeTableAlign(factory, tableHead, JcEnumeration.CENTER);		
					
					if(i3==0 && lList.get(i2).compareTo("Closed")==0)
					{
					Tr hodRowHeader = factory.createTr();
					addTableCell(factory, wordMLPackage, hodRowHeader, "LIST OF CLOSED SAFETY INCIDENTS", calibriBoldRPr, JcEnumeration.CENTER, true,"ffffff");
				...
				rowNum++;
					...
					int kl=i3+1;
				Tr hodRowHeader4 = factory.createTr();
				addTableCell(factory, wordMLPackage, hodRowHeader4, "1."+kl+".Work Name:"+closedWorkIdsList.get(i3)+" - "+closedWorkNameList.get(i3), calibriNotBoldRPr, JcEnumeration.LEFT, true,"ffffff");
				...
				if(closedWorkIdsList.size()>0)
				{
					if(!StringUtils.isEmpty(safetyData)) 
					{
						...
						for (Safety pObj : safetyData) 
						{
							...
							if(lList.get(i2).compareTo(pObj.getStatus_fk())==0  && pushcworkids.indexOf(pObj.getContract_id_fk())==-1)
							{
			...
			for (Training tObj : scheduledTrainings) {				
				...
				for (Training sObj : tObj.getTrainingSessions()) {	
					...
					addTableCell(factory, wordMLPackage, titleTableRow, "Title", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, tObj.getTitle(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					...
					addTableCell(factory, wordMLPackage, titleTableRow, "Description", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, tObj.getDescription(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "", garamondRPr,
							JcEnumeration.LEFT, false, null);
					...
					addTableCell(factory, wordMLPackage, titleTableRow, "Training Type", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, tObj.getTraining_type_fk(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "Date", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, sObj.getDate(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "", garamondRPr,
							JcEnumeration.LEFT, false, null);
					...
					addTableCell(factory, wordMLPackage, titleTableRow, "Category", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, tObj.getTraining_category_fk(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "Start Time", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, sObj.getStart_time(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "", garamondRPr,
							JcEnumeration.LEFT, false, null);
					...
					addTableCell(factory, wordMLPackage, titleTableRow, "Training Centre", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, tObj.getTraining_center(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "End Time", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, sObj.getEnd_time(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "", garamondRPr,
							JcEnumeration.LEFT, false, null);
		...
		if(!StringUtils.isEmpty(completedTrainings) && completedTrainings.size() > 0) {
			...
			for (Training tObj : completedTrainings) {				
				...
				for (Training sObj : tObj.getTrainingSessions()) {	
					...
					addTableCell(factory, wordMLPackage, titleTableRow, "Nominated", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, sObj.getNominated(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "Attended", garamondBoldRPr,
							JcEnumeration.LEFT, true, "ecf2ff");
					addTableCell(factory, wordMLPackage, titleTableRow, sObj.getAttended(), garamondRPr,
							JcEnumeration.LEFT, false, null);
					addTableCell(factory, wordMLPackage, titleTableRow, "", garamondRPr,
							JcEnumeration.LEFT, false, null);
	...
	public static void createTableForIssuesSummaryReport(WordprocessingMLPackage wordMLPackage, MainDocumentPart t,
		...
		if(!StringUtils.isEmpty(issuesCounts) && issuesCounts.size() > 0) {
			...
			for (Issue iObj : issuesCounts) {
				...
				addTableCell(factory, wordMLPackage, contentRow, String.valueOf(sNo++),
						garamondRPr, JcEnumeration.CENTER, hasBgColor, backgroundColor);
				addTableCell(factory, wordMLPackage, contentRow, iObj.getProject_id_fk(),
						garamondRPr, JcEnumeration.CENTER, hasBgColor, backgroundColor);				
	...
	public static void createTableForSummaryOfRiskAssessmentOfProjectsReport(WordprocessingMLPackage wordMLPackage,
		...
		try {
			...
			if(!StringUtils.isEmpty(top5RiskAreas)) {	
		        addPageBreak(mp);
		        addHeading(wordMLPackage, mp, factory,JcEnumeration.CENTER,calibriBold13RPr,"Top Five Risk Areas of WR Projects");
		        Tbl reportTable = factory.createTbl();
				addBorders(reportTable, "2");
