...
import com.synergizglobal.wrpmis.Iservice.ActivitiesUploadNewService;
...
public class ActivitiesUploadNewController {
	...
	@Autowired
	ActivitiesUploadNewService service;
					...
					try (XSSFWorkbook workbook = new XSSFWorkbook(multipartFile.getInputStream())) {					
						...
						if (sheetsCount > 2) {
							try {
								...
								if (headerRow != null) {
									List<String> fileFormat = FileFormatModel.getActivityRefetenceDataStructure_FileFormat();
							...
							for(int i = 2; i<= referenceDataSheet.getLastRowNum();i++){
								...
								if(!StringUtils.isEmpty(row)) {
									...
									if(!StringUtils.isEmpty(componentId_temp) && !componentId_temp.equals("null")) {									
										...
										String tempVal = formatter.formatCellValue(row.getCell(3)).trim();
										int count = org.apache.commons.lang3.StringUtils.countMatches(tempVal, "$");
										if(count != 2) {
											component_id = getCellDataType(workbook,row.getCell(3));
										}	
										...
										if(count != 2) {
											order_y = getCellDataType(workbook,row.getCell(4));
										}	
										if(!StringUtils.isEmpty(order_y)) { componentIdObj.setOrder_y(order_y);}
								...
								if(!StringUtils.isEmpty(row)) {
									...
									if(!StringUtils.isEmpty(componentId_temp) && !componentId_temp.equals("null")) {									
										...
										for (Activity cObj : componentIdList) {
											if(cObj.getComponent_id().equals(activityObj.getComponent())) {
