	...
	public List<Contract> getProcurementStatus(Contract obj) throws Exception {
		...
		try {
			String qry = "select * from ((select work_name,count(c.contract_id) as 'total',(select sum(ifnull(estimated_cost,0)*ifnull(estimated_cost_units,0))/(case when mu.unit='Cr' then 10000000 when mu.unit='L' then 100000 when mu.unit='Th' then 1000 when mu.unit='Rs' then 1 else 1 end)  \r\n"
					...
					+ "					left join department hoddt on u.department_fk = hoddt.department\r\n"
					+ "					left join user us on c.dy_hod_user_id_fk = us.user_id\r\n"
					+ "					where contract_id is not null and w.work_id='"+obj.getWork_id_fk()+"' ) as contract_details_types,\r\n"
					+ "                    \r\n"
					...
					+ "					left join department hoddt on u.department_fk = hoddt.department\r\n"
					+ "					left join user us on c.dy_hod_user_id_fk = us.user_id\r\n"
					+ "					where contract_id is not null and w.work_id='"+obj.getWork_id_fk()+"'  )   as awarded_cost,               \r\n"
					+ " ifnull((select group_concat(distinct structure_type SEPARATOR ', ') as Scope from activities_scurve s \r\n"
					+ "left join contract c on c.contract_id=s.contract_id \r\n"
					+ "where c.contract_id is not null and work_id='"+obj.getWork_id_fk()+"' ),'') as scope_of_contract\r\n"
					+ "\r\n"
					+ "                    \r\n"
					+ "                    from contract c  \r\n"
					+ "					left join work w on c.work_id_fk = w.work_id COLLATE utf8mb4_unicode_ci  \r\n"
					...
					+ "					left join user us on c.dy_hod_user_id_fk = us.user_id\r\n"
					+ "					where contract_id is not null and w.work_id='"+obj.getWork_id_fk()+"'  and hoddt1.department_name=hoddt.department_name) as contract_details_types,\r\n"
					+ "                    \r\n"
					...
					+ "					left join user us on c.dy_hod_user_id_fk = us.user_id\r\n"
					+ "					where contract_id is not null and w.work_id='"+obj.getWork_id_fk()+"'  and hoddt2.department_name=hoddt.department_name)   as awarded_cost,  \r\n"
					+ "                    \r\n"
					+ " ifnull((select group_concat(distinct structure_type SEPARATOR ', ') as Scope from activities_scurve s \r\n"
					+ "left join contract c on c.contract_id=s.contract_id \r\n"
					+ "					left join user u on c.hod_user_id_fk = u.user_id\r\n"
					...
					+ "					left join user us on c.dy_hod_user_id_fk = us.user_id\r\n"					
					+ "where c.contract_id is not null and work_id='"+obj.getWork_id_fk()+"'  and hoddt3.department_name=hoddt.department_name),'') as scope_of_contract\r\n"
					+ "\r\n"
					+ "                    \r\n"
					+ "                    from contract c  \r\n"
					+ "					left join work w on c.work_id_fk = w.work_id COLLATE utf8mb4_unicode_ci  \r\n"
