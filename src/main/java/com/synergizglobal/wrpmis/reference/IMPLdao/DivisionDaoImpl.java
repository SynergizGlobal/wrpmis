...
public class DivisionDaoImpl implements DivisionDao{
	...
	public List<TrainingType> getDivisionsList() throws Exception {
		...
		try {
			String qry ="select division_id, division_name, railway_zone from Divisions ";
	...
	public boolean addDivision(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO Divisions"
					+ "( division_id, division_name, railway_zone) VALUES (:division_id, :division_name, :railway_zone)";
	...
	public TrainingType getDivisionDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select division, division_name, railway_zone from Divisions ";
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as division_id,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from ["+bObj.getTable_name()+"] where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					String qry2 = "select division_id, division_name, railway_zone from divisions where division_id NOT IN (?";
		
					Object[] pValues = new Object[objsList1.size()];
					int j =0, p=1;
					for (TrainingType aObj : obj.getdList()) {
						pValues[j++] = aObj.getDivision_id();
						if( objsList1.size() >  p) {
							qry2 = qry2 + ",?";
							p++;
						}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='Division' and s.name='dbo'\r\n" + 
	...
	public boolean updateDivision(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table divisions NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE divisions SET division_id= :division_id_new,division_name= :division_name_new,railway_zone= :railway_zone_new WHERE division_id= :division_id_old " ;
			paramSource = new BeanPropertySqlParameterSource(obj);		 
			count = namedParamJdbcTemplate.update(updatereferenceTableQry, paramSource);	
			
			for (TrainingType bObj : obj.getdList()) {
				
				String updateTableQry = "UPDATE "+bObj.getTable_name()+" SET "+bObj.getColumn_name()+" =:division_id_new WHERE "+bObj.getColumn_name()+"= :division_id_old " ;
				
				 paramSource = new BeanPropertySqlParameterSource(obj);		 
				 namedParamJdbcTemplate.update(updateTableQry, paramSource);	
			}
			String  enableQry =	"Alter Table divisions CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteDivision(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE FROM divisions WHERE division_id= :division_id; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
