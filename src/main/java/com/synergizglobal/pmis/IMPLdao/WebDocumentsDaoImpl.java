...
public class WebDocumentsDaoImpl implements WebDocumentsDao{
	...
	public List<WebDocuments> getWebDocuments(WebDocuments obj) throws Exception {
		...
		try {
			...
			String docsQry ="select wd.id as web_document_id,title,file_name,category_id_fk,DATE_FORMAT(date_of_issue,'%d-%m-%Y') as date_of_issue,DATE_FORMAT(upload_date,'%d-%m-%Y') as upload_date,uploaded_by,"
