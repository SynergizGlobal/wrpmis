...
public class AlertTypeDaoImpl implements AlertTypeDao{
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='alert_type' and s.name='dbo'\r\n" + 
	...
	public boolean updateAlertType(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table alert_type NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table alert_type CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
