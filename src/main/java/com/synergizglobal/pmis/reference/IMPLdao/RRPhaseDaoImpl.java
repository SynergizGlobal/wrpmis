...
@Repository
public class RRPhaseDaoImpl implements RRPhaseDao{
	@Autowired
		...
		try {
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as phase,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select phase from rr_phase where phase NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getPhase();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select t1.name as tname,(select c2.name from sys.columns c2 where c2.object_id=t1.object_id\r\n" + 
					...
					"WHERE t.name = 'rr_phase' and s.name = 'dbo'),'%')) as COLUMN_NAME,'' as CONSTRAINT_NAME,\r\n" + 
					"'rr_phase' as REFERENCED_TABLE_NAME,\r\n" + 
					...
					"WHERE t2.name = 'rr_phase' and s2.name = 'dbo')) as REFERENCED_COLUMN_NAME\r\n" + 
					...
					"left join sys.tables t on t.object_id = c.object_id\r\n" + 
					"left join sys.schemas s on s.schema_id = t.schema_id\r\n" + 
					"WHERE t.name = 'rr_phase' and s.name = 'dbo'),'%'))\r\n" + 
					"and t1.name !=  'rr_phase'";
	...
	public boolean addRRPhase(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO rr_phase"
					+ "( phase) VALUES (:phase)";
	...
	public boolean updateRRPhase(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE rr_phase SET phase= :value_new WHERE phase= :value_old " ;
	...
	public boolean deleteRRPhase(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from rr_phase WHERE phase= :phase; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
