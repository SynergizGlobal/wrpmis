...
public class WebviewContractResourceController {

	@InitBinder
    public void initBinder(WebDataBinder binder) {
        binder.registerCustomEditor(String.class, new StringTrimmerEditor(true));
    } 
	Logger logger = Logger.getLogger(WebviewContractResourceController.class);
	...
	public ModelAndView contractResourceForm(@ModelAttribute ContractResource obj,HttpSession session,RedirectAttributes attributes) {
		ModelAndView model = new ModelAndView(MobilePageConstants2.resourcesGrid);
		try {		
	...
	public ModelAndView addResource(@ModelAttribute ContractResource obj,HttpSession session,RedirectAttributes attributes) {
		...
		try {
			model.setViewName("redirect:/mobileappwebview/contract-resource-form");
