	...
	public List<Contract> getProjectOverviewReportList(Contract obj) throws Exception {
		...
		try {
			String qry = "select * from (SELECT distinct contract_short_name,work_name,\r\n"
					...
					+ ")\r\n"
					+ " ,0) as actual_financial_progress,case when c.contract_id like '%"+obj.getWork_id_fk()+"%' and (contract_name like '%Expense - Miscellaneous%' or contract_name like '%Expense - Land%') then 'NBF' else d1.department end department,\r\n"
					+ "                    ifnull((case when (case when cr.revised_amount is null then awarded_cost*awarded_cost_units else revised_amount*revised_amount_units end) is null then (estimated_cost*estimated_cost_units) else (case when cr.revised_amount is null then awarded_cost*awarded_cost_units else revised_amount*revised_amount_units end) end),0)-ifnull((SUM((e.gross_work_done * e.gross_work_done_units))),0) AS actual_physical_progress,case when c.contract_id like '%"+obj.getWork_id_fk()+"%' and (contract_name like '%Expense - Miscellaneous%' or contract_name like '%Expense - Land%') then 'Non Bank Funds' else d1.department_name end department_name \r\n"
