...
public class OverviewDashboardDaoImpl implements OverviewDashboardDao {
	...
	public List<OverviewDashboard> getLeftNavNodes(OverviewDashboard dObj) throws Exception {
		...
		try {
			...
			while(resultSet.next()) {
				...
				List<OverviewDashboard> subList = getDashboardsSubList(dObj,project_id,childParentId,connection);
				...
				if(!StringUtils.isEmpty(project_id)) {
					obj.setSource_field_value(project_id);
				}
	...
	private List<OverviewDashboard> getDashboardsSubList(OverviewDashboard dObj, String project_id, String parentId, Connection connection) throws Exception {
		PreparedStatement statement = null;
		ResultSet resultSet = null;
		List<OverviewDashboard> objsList = new ArrayList<OverviewDashboard>();
			...
			for (OverviewDashboard obj : objList) {		
				if(!StringUtils.isEmpty(obj.getIs_first_option_selected()) && (obj.getIs_first_option_selected()).toUpperCase().equals("YES")){
					if(!StringUtils.isEmpty(obj.getQuery_for_filter_options())){
						...
						if(!StringUtils.isEmpty(tempObj) && !StringUtils.isEmpty(tempObj.getSource_field_name()) && !StringUtils.isEmpty(dObj.getProject_id())) {
	...
	public int getWorkDroneSurveyCount(String project_id) throws Exception {
		...
		try {
			...
			count = (int) jdbcTemplate.queryForObject(qry, new Object[] { project_id } ,int.class);
		} catch (Exception e) {
			throw new Exception(e);
		}		
		return count;
	...
	public List<OverviewDashboard> getContractorRoleWorks(String user_name) throws Exception {
		...
		try {
			String qry = "SELECT distinct project_id from p6_activities a  " + 
	...
	public List<OverviewDashboard> getContractorRoleWorkContracts(String user_name) throws Exception {
		List<OverviewDashboard> objsList = null;
		try {
			String qry = "SELECT distinct contract_id_fk as filter_id,contract_short_name as filter_label_name from p6_activities a  " + 
					"LEFT JOIN contract c ON a.contract_id_fk = c.contract_id  " + 
					"left join project p on c.project_id_fk = p.project_id  " + 
					"left join contractor c1 on c1.contractor_id=c.contractor_id_fk " + 
					"where contract_id_fk is not null and contract_id_fk <> '' and contractor_name=?";
	...
	public String getDashboardURL(String dashboardId, String projectId) {
	    ...
	    try {
	        ...
	        String sql = "SELECT dashboard_url FROM left_menu WHERE dashboard_id = ?";

	        ps = connection.prepareStatement(sql);
	        ps.setString(1, dashboardId);

	        rs = ps.executeQuery();

	        if (rs.next()) {
	            dashboardURL = rs.getString("dashboard_url");
	        }

	    } catch (Exception e) {
	        e.printStackTrace();
	        throw new RuntimeException("Error retrieving dashboard URL", e);
	    } finally {
