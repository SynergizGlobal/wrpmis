...
public class RRLocationDaoImpl implements RRLocationDao{
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='rr_location' and s.name='dbo'\r\n" + 
	...
	public boolean updateRRLocation(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table rr_location NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			...
			String  enableQry =	"Alter Table rr_location CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
