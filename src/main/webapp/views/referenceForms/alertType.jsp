                        ...
                        <div class="row">
                            ...
                            <div class="col m4 s12 center-align">
                                <a class="waves-effect waves-light btn bg-m modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add Alert Type</a>
                            </div>
                            <div class="col m4 hide-on-small"></div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="alert_type_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                            <th>Alert Type</th>
                                             <c:forEach var="tObj" items="${alertTypeDetails.tablesList}" >
										...
										<c:forEach var="obj" items="${alertTypeDetails.dList1}" varStatus="indexs" >
											...
											<c:forEach var="tObj" items="${alertTypeDetails.tablesList}" varStatus="index">
												...
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    		...
													    		<c:choose>  
																    <c:when test="${cObj.alert_type eq obj.alert_type }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
											...
											<td class="last-column "><a onclick="updateRow(${indexs.count})" class="btn waves-effect waves-light bg-m t-c "> <i class="fa fa-pencil" ></i></a>
										 	<c:forEach var="oSbj"  items="${alertTypeDetails.dList}" varStatus="indexx"> 
												...
												<c:choose>  
												    <c:when test="${oSbj.alert_type eq obj.alert_type }"> 
												      	<a onclick="deleteRow('${ oSbj.alert_type }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
                ...
                <div class="row">
                    ...
                    <div class="col m8 s12">
                        ...
                        <div class="row">
                            ...
                            <div class="col s12 m6">
                                <div class="center-align m-1">
                                    <a href="<%=request.getContextPath()%>/alert-type"
...
<%--   <jsp:include page="../layout/footer.jsp"></jsp:include> --%>
    ...
    <script>
        $(document).ready(function () {
            ...
            var table = $('#alert_type_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
       ...
       $("#updateAlertTypeForm").submit(function (e) {
        ...
        var validator =	$('#alertTypeForm').validate({
       	 rules: {
        	 ...
        	 if(element.attr("id") == "alert_type_text" ){
			     document.getElementById("alert_typeError").innerHTML="";
		 	     error.appendTo('#alert_typeError');
			 }
        }
      });
      var validator1 = $('#updateAlertTypeForm').validate({
        ...
        function updateRow(no) {
            var alert_type = $('#alert_typeId'+no).val();
            $('#value_old').val($.trim(alert_type))
            $('#onlyUpdateModal').modal('open');
            $('#onlyUpdateModal #value_new').val($.trim(alert_type)).focus();
        }
          	...
          	swal({
      	            ...
      	            if (isConfirm) {
      	               // swal("Deleted!", "Record has been deleted", "success");
      	                $(".page-loader").show();
      	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-alert-type');
      	    	    	$('#getForm').submit();
      	           }else {
