...
@Repository
public class UserLoginEventTypeDaoImpl implements UserLoginEventTypeDao{
	@Autowired
	...
	public TrainingType getUserTypeDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select `login_event_type` from user_login_event_type ";
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as `login_event_type`,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select `login_event_type` from user_login_event_type where `login_event_type` NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getLogin_event_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'user_login_event_type' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'user_login_event_type' and TABLE_SCHEMA = 'pmis'";
	...
	public boolean addUserLoginEventType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO user_login_event_type"
					+ "( login_event_type) VALUES (:login_event_type)";
	...
	public boolean updateUserLoginEventType(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE user_login_event_type SET `login_event_type`= :value_new WHERE `login_event_type`= :value_old " ;
	...
	public boolean deleteUserLoginEventType(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from user_login_event_type WHERE `login_event_type`= :login_event_type; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
