...
@Repository
public class DataGatheringRefrenceDaoImpl implements DataGatheringsDao{
	...
	public List<TrainingType> getDataGatheringsList() throws Exception {
		...
		try {
			String qry ="select status from data_gathering_status ";
	...
	public boolean addDataGatherings(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO data_gathering_status"
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select status from data_gathering_status where status NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'data_gathering_status' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'data_gathering_status' and TABLE_SCHEMA = 'pmis' ";
	...
	public boolean updateDataGatheringStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE data_gathering_status SET status= :status_new WHERE status= :status_old " ;
	...
	public boolean deleteDataGatheringStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from data_gathering_status WHERE status= :status; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
