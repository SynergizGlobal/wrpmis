...
@Repository
public class PMISCategoryDaoImpl implements PMISCategoryDao{
	...
	public List<Safety> getCategoryList() throws Exception {
		...
		try {
			String qry ="select category from pmis_category ";
	...
	public boolean addCategory(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO pmis_category"
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select category from pmis_category where category NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'pmis_category' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'pmis_category' and TABLE_SCHEMA = 'pmis' ";
	...
	public boolean updatePmisCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE pmis_category SET category= :value_new WHERE category= :value_old " ;
	...
	public boolean deletePmisCategory(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from pmis_category WHERE category= :category; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
