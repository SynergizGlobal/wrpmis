...
import com.synergizglobal.wrpmis.common.DocxTableCreationForContractReport;
...
public class FinanceReportController {
	...
	@RequestMapping(value="/finance-summary-report",method= {RequestMethod.GET,RequestMethod.POST})
	public void financeSummaryReport(HttpSession session,HttpServletResponse response,RedirectAttributes attributes) {
		ModelAndView model = new ModelAndView();
		try{
		
				ResultSet rs=projectWorkOverviewReportService.getFinanceSummaryReport(); 
                List<Contract> rsChild=projectWorkOverviewReportService.getFinanceSummaryReportByProjectWorks();
	                ...
	                for (Contract cObj : rsChild) 
	                {
	                	...
	                	if(ProjectName.compareTo(WorkProjectName)==0)
	                	{
	                		...
	                		XSSFRow row11 = executionOverviewReportSheet.createRow(RowNum);
		                    	...
		                    	if(!StringUtils.isEmpty(cObj.getCumulative_expenditure()) && cObj.getCumulative_expenditure()!="")
		                    	{
			                    	if(Float.parseFloat(cObj.getCumulative_expenditure())>0)
			                    	{
			                    		cell.setCellValue(cObj.getCumulative_expenditure());
			                    	}
			                    	else
			                    	{
			    						cell.setCellValue("");
		                    	...
		                    	if(!StringUtils.isEmpty(cObj.getBg_value()) && cObj.getBg_value()!="")
		                    	{
			                    	if(Float.parseFloat(cObj.getBg_value())>0)
			                    	{
			                    		cell.setCellValue(cObj.getBg_value());
			                    	}
			                    	else
			                    	{
			    						cell.setCellValue("");
		                    	...
		                    	if(!StringUtils.isEmpty(cObj.getValue()) && cObj.getValue()!="")
		                    	{
			                    	if(Float.parseFloat(cObj.getValue())>0)
			                    	{
			                    		cell.setCellValue(cObj.getValue());
			                    	}
			                    	else
			                    	{
			    						cell.setCellValue("");
		                    	...
		                    	if(!StringUtils.isEmpty(cObj.getInsurance_value()) && cObj.getInsurance_value()!="")
		                    	{
			                    	if(Float.parseFloat(cObj.getInsurance_value())>0)
			                    	{
			                    		cell.setCellValue(cObj.getInsurance_value());
			                    	}
			                    	else
			                    	{
			    						cell.setCellValue("");
		                    	...
		                    	if(!StringUtils.isEmpty(cObj.getExpenditure()) && cObj.getExpenditure()!="")
		                    	{
			                    	if(Float.parseFloat(cObj.getExpenditure())>0)
			                    	{
			                    		cell.setCellValue(cObj.getExpenditure());
			                    	}
			                    	else
			                    	{
			    						cell.setCellValue("");
		                    	...
		                    	if(!StringUtils.isEmpty(cObj.getEstimated_cost()) && cObj.getEstimated_cost()!="")
		                    	{
			                    	if(Float.parseFloat(cObj.getEstimated_cost())>0)
			                    	{
			                    		cell.setCellValue(cObj.getEstimated_cost());
			                    	}
			                    	else
			                    	{
			    						cell.setCellValue("");
                ...
                Date date = new Date();
                String fileName = "Financial Summary Report "+dateFormat.format(date);
