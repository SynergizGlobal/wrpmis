...
public class P6DataController {
	...
	@RequestMapping(value = "/upload-p6data", method = {RequestMethod.POST})
	public ModelAndView uploadP6Data(@ModelAttribute P6Data p6data,RedirectAttributes attributes,HttpSession session){
				...
				if (null != multipartFile && multipartFile.getSize() > 0){					
					...
					if(workbook != null) {
						...
						if(sheetsCount > 0) {
							...
							int count = p6WbsUpload(p6data,userId,userName,workbook);
	...
	private int p6WbsUpload(P6Data obj, String userId, String userName,XSSFWorkbook workbook) throws Exception {
		P6Data p6data = null;
					...
					if(workbook != null && !"".equals(workbook)) {
						...
						if(sheetsCount > 0) {
							...
							for(int i = 2; i<= uploadFilesSheet.getLastRowNum();i++){
								...
								if(!StringUtils.isEmpty(p6data) && !StringUtils.isEmpty(p6data.getContract_id_fk()) && !StringUtils.isEmpty(p6data.getFob_id_fk())) {
									p6dataList.add(p6data);
								}
							}
							
							if(!p6dataList.isEmpty() && p6dataList != null){
								count  = p6dataService.p6WbsUpload(p6dataList);
							}
						}
						workbook.close();
					}
					...
					if(workbook != null && !"".equals(workbook)) {
						...
						if(sheetsCount > 0) {
							...
							for(int i = 2; i<= uploadFilesSheet.getLastRowNum();i++){
								...
								if(!StringUtils.isEmpty(p6data) && !StringUtils.isEmpty(p6data.getP6_wbs_code_fk()) && !StringUtils.isEmpty(p6data.getP6_activity_name())) {
									p6dataList1.add(p6data);
								}
							}
							
							if(!p6dataList1.isEmpty() && p6dataList1 != null){
								count  = p6dataService.p6ActivitiesUpload(p6dataList1);
							}
						}
						workbook.close();
					}
