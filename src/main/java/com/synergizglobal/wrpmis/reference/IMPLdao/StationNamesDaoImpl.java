...
@Repository
public class StationNamesDaoImpl implements StationNamesDao{
	...
	public TrainingType getStationNamesDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select station_name from stationnames ";
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as station_name,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select station_name from stationnames where station_name NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getStation_name();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select  object_name(fkx.fkeyid) as tName,\r\n" + 
					...
					"where object_name(fkx.rkeyid)='station_name' and s.name='dbo'\r\n" + 
	...
	public boolean addStationNames(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO stationnames"
	...
	public boolean updateStationNames(TrainingType obj) throws Exception {
		...
		try {
			...
			String disableQry = "Alter Table stationnames NOCHECK Constraint All ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
			namedParamJdbcTemplate.update(disableQry, paramSource);	
			
			String  updatereferenceTableQry = "UPDATE stationnames SET station_name= :value_new WHERE station_name= :value_old " ;
			...
			String  enableQry =	"Alter Table stationnames CHECK Constraint All";
			paramSource = new BeanPropertySqlParameterSource(obj);	
	...
	public boolean deleteStationNames(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from stationnames WHERE station_name= :station_name; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
