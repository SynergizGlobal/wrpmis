...
@Repository
public class RRMaritalStatusDaoImpl  implements RRMaritalStatusDao{
	@Autowired
	...
	public TrainingType getRRMaritalStatusDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select marital_status from marital_status ";
			...
			if(tablesList.size() > 0) {
				...
				for (TrainingType bObj : obj.getdList()) {
					...
					qry1 = qry1 +"select "+bObj.getColumn_name()+" as marital_status,count("+bObj.getColumn_name()+") as count,'"+bObj.getTable_name()+"' as tName from "+bObj.getTable_name()+" where "+bObj.getColumn_name()+" <> '' group by "+bObj.getColumn_name()+"  ";
					if( list.size() >  i) {
						qry1 = qry1 + " UNION ";
						i++;
					}
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select marital_status from marital_status where marital_status NOT IN (?";
	
						int j =0, p=1;
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getMarital_status();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select t1.name as tname,(select c2.name from sys.columns c2 where c2.object_id=t1.object_id\r\n" + 
					...
					"WHERE t.name = 'marital_status' and s.name = 'dbo'),'%')) as COLUMN_NAME,'' as CONSTRAINT_NAME,\r\n" + 
					"'marital_status' as REFERENCED_TABLE_NAME,\r\n" + 
					...
					"WHERE t2.name = 'marital_status' and s2.name = 'dbo')) as REFERENCED_COLUMN_NAME\r\n" + 
					...
					"left join sys.tables t on t.object_id = c.object_id\r\n" + 
					"left join sys.schemas s on s.schema_id = t.schema_id\r\n" + 
					"WHERE t.name = 'marital_status' and s.name = 'dbo'),'%'))\r\n" + 
					"and t1.name !=  'marital_status'";
	...
	public boolean addRRMaritalStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO marital_status"
	...
	public boolean updateRRMaritalStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE marital_status SET marital_status= :value_new WHERE marital_status= :value_old " ;
	...
	public boolean deleteRRMaritalStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from marital_status WHERE marital_status= :marital_status; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
