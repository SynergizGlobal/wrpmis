    ...
    <style>
      ...
      /*   .row.no-mar {
		...
		textarea.materialize-textarea:focus:not([readonly])   {
		    border-bottom: 1px solid #637c83 !important;
		    box-shadow: 0 1px 0 0 #637c83 !important;
		}
...
<%-- <jsp:include page="../layout/header.jsp"></jsp:include> --%>
                        ...
                        <div class="row">
                            ...
                            <div class="col m4 s12 center-align">
                                <a class="waves-effect waves-light btn bg-m modal-trigger t-c" href="#addUpdateModal">
                                    <i class="fa fa-plus-circle"></i> &nbsp; Add Issue Status</a>
                            </div>
                            <div class="col m4 hide-on-small"></div>
                        </div>
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="issue_status_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                            <th>Issue Status</th>
                                            <c:forEach var="tObj" items="${issueStatusDetails.tablesList}" >
                                    ...
                                    <tbody>
										<c:forEach var="obj" items="${issueStatusDetails.dList1}" varStatus="indexs">
											<tr><td>
												<input type="hidden" id="statusId${indexs.count}" value="${obj.status }" class="findLengths" />
												${obj.status }</td>
											<c:forEach var="tObj" items="${issueStatusDetails.tablesList}" varStatus="index">
												 
												<td><c:forEach var="cObj" items="${issueStatusDetails.countList}" >
												<c:choose> 
											...
											<td class="last-column "><a onclick="updateRow(${indexs.count})" class="btn waves-effect waves-light bg-m t-c modal-trigger"> <i class="fa fa-pencil" ></i></a>
										 	<c:forEach var="oSbj"  items="${issueStatusDetails.dList}" varStatus="indexx"> 
    ...
    <div id="addUpdateModal" class="modal">
		<form action="<%=request.getContextPath() %>/add-issue-status" id="addStatusForm" name="addStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header">Add Issue Status <span class="right modal-action modal-close"><span
                    ...
                    <div class="col m8 s12">
                        <div class="row">
                            <div class="input-field col s12 m12">
                                <input id="issue_status_text" name="status" type="text" class="validate"  onkeyup="doValidate(this.value)">
                                <label for="issue_status_text">Issue Status</label>
                            ...
                            <div class="col s12 m6">
                                <div class="center-align m-1">
                                  <!--   <button
                                        ...
                                        <a href="<%=request.getContextPath()%>/issue-status"
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-issue-status" id=updateStatusForm name="updateStatusForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header bg-m">Update Issue Status <span class="right modal-action modal-close" onclick="removeErrorMsg()"><span
                            class="material-icons">close</span></span></h5>
                <div class="row">
                    <div class="col m2 hide-on-small"></div>
                    <div class="col m8 s12">
                       <div class="row no-mar">
                         <div class="input-field col s12 m12">
                                ...
                                <label for="status_new">Issue Status</label>
    ...
    <script>
        $(document).ready(function () {
           ...
           /*  // adding table data into table start
            var arr = ['Assigned', 'Closed', 'Escalated', 'Raised'];
            var table_text = '';
            ...
            $('#issue_status_table tbody').append(table_text);
            // adding table data into table ends
 */
            var table = $('#issue_status_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
         ...
         var validator =  $('#addStatusForm').validate({
         	 rules: {
         		 "status": {
   	        	 ...
   	        	 if(element.attr("id") == "issue_status_text" ){
        	...
        	swal({
    	            ...
    	            if (isConfirm) {
    	               // swal("Deleted!", "Record has been deleted", "success");
    	                $(".page-loader").show();
    	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-issue-status');
    	    	    	$('#getForm').submit();
    	           }else {
