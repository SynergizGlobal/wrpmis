...
import com.synergizglobal.pmis.reference.Idao.P6WbsCategoryDao;
import com.synergizglobal.pmis.reference.model.Safety;
import com.synergizglobal.pmis.reference.model.TrainingType;

@Repository
public class P6WbsCategoryDaoImpl implements P6WbsCategoryDao{
	...
	public boolean addP6WbsCategory(Safety obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO p6_wbs_category"
					+ "( p6_wbs_category) VALUES (:p6_wbs_category)";
					...
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select `p6_wbs_category` from p6_wbs_category where `p6_wbs_category` NOT IN (?";
						...
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getP6_wbs_category();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'p6_wbs_category' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'p6_wbs_category' and TABLE_SCHEMA = 'pmis' ";
