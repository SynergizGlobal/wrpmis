...
import com.synergizglobal.wrpmis.Idao.FOBDao;
...
public class FOBDaoImpl implements FOBDao {
	...
	public FOB getFOB(FOB obj) throws Exception {
		...
		try {
			...
			if(!StringUtils.isEmpty(fobj) && !StringUtils.isEmpty(fobj.getFob_id())) {
				...
				qryFOBDetail = qryFOBDetail + " and detail_name in('Type','Location of FOB','KM','FOB Length (m)','FOB Width (m)','Platforms Connecting','No. of Staircases','No. of Skywalk','Future Provision Escalators nos') order by case when detail_name='Type' then 1\r\n" + 
						"when detail_name='Location of FOB' then 2\r\n" + 
						"when detail_name='KM' then 3\r\n" + 
						"when detail_name='FOB Length (m)' then 4\r\n" + 
						"when detail_name='FOB Width (m)' then 5\r\n" + 
						"when detail_name='Platforms Connecting' then 6\r\n" + 
						"when detail_name='No. of Staircases' then 7\r\n" + 
						"when detail_name='No. of Skywalk' then 8\r\n" + 
						"when detail_name='Future Provision Escalators nos' then 9 end asc";
				...
				if(StringUtils.isEmpty(objsList) || objsList.size() == 0) {
					...
					qryFOBDetail = qryFOBDetail + " where detail_name in('Type','Location of FOB','KM','FOB Length (m)','FOB Width (m)','Platforms Connecting','No. of Staircases','No. of Skywalk','Future Provision Escalators nos') "
							+ "group by detail_name order by case when detail_name='Type' then 1\r\n" + 
							"when detail_name='Location of FOB' then 2\r\n" + 
							"when detail_name='KM' then 3\r\n" + 
							"when detail_name='FOB Length (m)' then 4\r\n" + 
							"when detail_name='FOB Width (m)' then 5\r\n" + 
	...
	public List<FOB> getContractsListForFilter(FOB obj) throws Exception {
		...
		try {
			...
			qry = qry + "GROUP BY ce.contract_id_fk,c.contract_name,c.contract_short_name ";
			Object[] pValues = new Object[arrSize];
			int i = 0;
			if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getWork_status_fk())){
				pValues[i++] = obj.getWork_status_fk();
