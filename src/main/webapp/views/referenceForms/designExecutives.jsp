...
<body>
                    ...
                    <div class="">
                        ...
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="design_executives_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                            <th>Work</th>
                                            <th>Executives</th>
                                    ...
                                    <c:forEach var="obj" items="${executivesDetails}" varStatus="indexs">
											<tr>
											<td>
												<input type="hidden" id="design_work_id_fk${indexs.count}" value="${obj.work_id_fk }" /> 
												${obj.work_short_name }</td>
											<td>
												<input type="hidden" id="design_user_id${indexs.count}" value="${obj.user_id }" /> 
												${obj.user_name }</td>
   ...
   <div class="page-loader" style="display: none;">
    ...
    <div id="addUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/add-design-executives" id="addRrExecutivesForm" name="addRrExecutivesForm" method="post" class="form-horizontal" role="form">
                        		...
                        		<div class="row">
		                            ...
		                            <div class="col s12 m6">
		                                <div class="center-align m-1">
		                                         ...
		                                         <a href="<%=request.getContextPath()%>/design-executives"
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-design-executives" id=updateRrExecutivesForm name="updateRrExecutivesForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
...
<%-- <jsp:include page="../layout/footer.jsp"></jsp:include> --%>
	<form name="getForm" id="getForm" method="post">
    	<input type="hidden" name="design_sub_location" id="id" />
    </form>
    ...
    <script>
        $(document).ready(function () {
          ...
          /*   
            ...
            var table = $('#design_executives_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
        ...
        function getResponsibleUsers(work_id)
        {
		  ...
		  $.ajax({
              url: "<%=request.getContextPath()%>/ajax/getWorkWiseDesignResponsibleUsers",
    ...
    /*    var validator = $('#addRrExecutivesForm').validate({
        ignore: ":hidden:not(.validate-dropdown)",
      	 rules: {
  	        ...
  	        },edesignorPlacement:function(edesignor, element){
  	        	 if(element.attr("id") == "design_location_fk" ){
  				     document.getElementById("design_location_fkEdesignor").innerHTML="";
  			 	     edesignor.appendTo('#design_location_fkEdesignor');
  			   }else if(element.attr("id") == "design_sub_location" ){
  				     document.getElementById("design_sub_locationEdesignor").innerHTML="";
  			 	     edesignor.appendTo('#design_sub_locationEdesignor');
  			   }
  	        }
       });
       ...
       var validator1 = $('#updateRrExecutivesForm').validate({
           ignore: ":hidden:not(.validate-dropdown)",
         	 rules: {
     	        	 ...
     	        	 if(element.attr("id") == "design_location_fk_new" ){
     				     document.getElementById("design_location_fk_newEdesignor").innerHTML="";
     			 	     edesignor.appendTo('#design_location_fk_newEdesignor');
     			   }else if(element.attr("id") == "value_new" ){
     				     document.getElementById("value_newEdesignor").innerHTML="";
     			 	     edesignor.appendTo('#value_newEdesignor');
     			   }
     	        }
          }); */
      ...
      function updateRow(no) {
          ...
          var work_id_fk = $('#design_work_id_fk'+no).val();
         $('#work_id_fk_old').val(work_id_fk);
          var user_id = $('#design_user_id'+no).val();
    				 ...
    				 /*   <c:choose>
		    	        ...
		    	        /*  </c:when>
		    	         ...
		    	         '<select  class="searchable validate-dropdown" name="work_id_fks" id="work_id_fk'+rNo+'" >'+
		    	         		 '<option value="">Select</option>'+
		    	                 <c:forEach var="obj" items="${usersDetails}">
		    				 		'<option  value="${obj.work_id_fk }">${obj.work_id_fk } - ${obj.design_short_name }</option>'
		    	                 </c:forEach>
		    	         +'</select>' +                               
		    	         </c:otherwise>
		    	         </c:choose>			   */ 
