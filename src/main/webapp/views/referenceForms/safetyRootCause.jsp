											...
											<c:forEach var="tObj" items="${safetyRootCauseDetails.tablesList}" varStatus="index">
												...
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    		<c:choose>  
																    <c:when test="${cObj.root_cause eq obj.root_cause }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
         ...
         var validator =  $('#addSafetyRootForm').validate({
         	 rules: {
         		 "root_cause": {
  	        	 ...
  	        	 if(element.attr("id") == "safety_root_cause_text" ){
  				     document.getElementById("root_causeError").innerHTML="";
  			 	     error.appendTo('#root_causeError');
  				 }
  	        }
         	
         });
         
         var validator1 =  $('#updateSafetyRootForm').validate({
           	 rules: {
           		 "value_new": {
        			 		  required: true
           		 }
            ...
            function updateRow(no) {
        	      var root_cause = $('#root_causeId'+no).val();
        	      $('#value_old').val($.trim(root_cause))
        	      $('#onlyUpdateModal').modal('open');
        	      $('#onlyUpdateModal #value_new').val($.trim(root_cause)).focus();
        	  }
        	    	...
        	    	swal({
        		            ...
        		            if (isConfirm) {
        		               // swal("Deleted!", "Record has been deleted", "success");
        		            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-safety-root-cause');
        		    	    	$('#getForm').submit();
        		           }else {
