...
public class UtilityShiftingDaoImpl implements UtilityShiftingDao {
	...
	private String getAutoGeneratedUSId(UtilityShifting obj) {
		...
		String utilityshiftingid = obj.getWork_code()+"-US-01";
		List<UtilityShifting> objsList = null;
		try {
			...
			String qry2="select CONCAT('"+obj.getWork_code()+"','-US-',case when len(utility_shifting_id)=3 then concat('0',utility_shifting_id) when len(utility_shifting_id)=2 then concat('00',utility_shifting_id) when len(utility_shifting_id)=1 then concat('000',utility_shifting_id) end) as utilityshiftingid from(\r\n" + 
