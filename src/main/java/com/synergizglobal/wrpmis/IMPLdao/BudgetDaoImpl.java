...
public class BudgetDaoImpl implements BudgetDao {
	...
	public List<Budget> budgetList(Budget obj) throws Exception {
		...
		try {
			...
			String qry ="select budget_id,p.project_id,p.project_name,max(b.financial_year_fk) as financial_year_fk,cast(budget_estimate as CHAR) as budget_estimate,cast(budget_grant as CHAR) as budget_grant, cast(revised_estimate as CHAR) as revised_estimate,cast(revised_grant as CHAR) as revised_grant,cast(final_estimate as CHAR) as final_estimate,cast(final_grant as CHAR) as final_grant,cast(target as CHAR) as target  from budget b LEFT JOIN work w on b.work_id_fk = w.work_id LEFT JOIN financial_year f on b.financial_year_fk = f.financial_year LEFT JOIN project p on  w.project_id_fk = p.project_id WHERE b.financial_year_fk = (SELECT (CASE WHEN MONTH(CONVERT(date, getdate())) >= 4 THEN concat(YEAR(CONVERT(date, getdate())), '-',SUBSTRING(cast(YEAR(CONVERT(date, getdate()))+1 as varchar),3,2)) ELSE concat(cast(YEAR(CONVERT(date, getdate()))-1 as varchar),'-', \r\n" + 
			...
			qry = qry +" group by budget_id,p.project_id,p.project_name,budget_estimate,budget_grant,revised_estimate,revised_grant,final_estimate,final_grant ";
	...
	public Budget getBudget(Budget obj)throws Exception{
		Budget budget = null;
		try {
			String qry = "select budget_id, p.project_name,b.project_id_fk from budget b " + 
					"left join project p on b.project_id_fk = p.project_id where budget_id is not null" ; 
			int arrSize = 0;
			...
			if(!StringUtils.isEmpty(budget) && !StringUtils.isEmpty(budget.getBudget_id())) {
				...
				String qryDetails = "select budget_id,b.financial_year_fk,budget_estimate, revised_estimate, final_estimate,"+
						...
						+ "from budget b " 
						+"left join financial_year f on b.financial_year_fk = f.financial_year where project_id_fk = ?  ORDER BY financial_year_fk DESC";
				
				objsList = jdbcTemplate.query(qryDetails, new Object[] {budget.getProject_id_fk()}, new BeanPropertyRowMapper<Budget>(Budget.class));	
	...
	public boolean addBudget(Budget budget) throws Exception {
		...
		try {
			...
			String insert_qry = "INSERT into  budget ( project_id_fk, financial_year_fk, budget_estimate, revised_estimate, "
			...
			if(!StringUtils.isEmpty(budget.getFinancial_year_fks()) && budget.getFinancial_year_fks().length > 0) {
				for (int i = 0; i < arraySize; i++) {
				    ...
				    if( budget.getFinancial_year_fks().length > 0 && !StringUtils.isEmpty(budget.getFinancial_year_fks()[i])) {
					    insertStmt.setString(p++,(budget.getProject_id_fk()));
	...
	public boolean updateBudget(Budget budget) throws Exception {
		...
		try {
			...
			con.setAutoCommit(false);
			con.setAutoCommit(false);
			String deleteQry = "DELETE from budget_files  where project_id_fk = ?";		 
			stmt = con.prepareStatement(deleteQry);
			stmt.setString(1,budget.getProject_id_fk());
			stmt.executeUpdate();
			if(stmt != null){stmt.close();}
			
			String inactiveQry = "DELETE from budget  where project_id_fk = ?";		 
	...
	public List<Budget> getBudgetProjectsList(Budget obj) throws Exception {
		...
		try {
			String qry = "SELECT p.project_id as project_id_fk,p.project_name from budget b " + 
					"LEFT JOIN project p on b.project_id_fk = p.project_id  " + 
	...
	public List<Budget> getBudgetExportList(Budget obj) throws Exception {
		List<Budget> objsList = null;
		try {
			String qry ="SELECT budget_id,p.project_id,p.project_name,b.financial_year_fk,cast(budget_estimate as CHAR) as budget_estimate,cast(budget_grant as CHAR) as budget_grant, " 
					...
					+ "LEFT JOIN project p on  b.project_id_fk = p.project_id "
					+ "WHERE b.financial_year_fk = (SELECT (CASE WHEN MONTH(CONVERT(date, getdate())) >= 4 THEN concat(YEAR(CONVERT(date, getdate())), '-',SUBSTRING(cast(YEAR(CONVERT(date, getdate()))+1 as varchar),3,2)) ELSE concat(YEAR(CONVERT(date, getdate()))-1,'-', SUBSTRING(cast(YEAR(CONVERT(date, getdate())) as varchar),3,2)) END) AS financial_year) " 
			...
			qry = qry +" order BY financial_year_fk desc ";
	...
	public int getTotalRecords(Budget obj, String searchParameter) throws Exception {
		int totalRecords = 0;
		try {
			String qry ="select count(DISTINCT project_id_fk) as total_records from budget b "
					+ "LEFT JOIN financial_year f on b.financial_year_fk = f.financial_year " 
					+ "LEFT JOIN project p on  b.project_id_fk = p.project_id "
			...
			if(!StringUtils.isEmpty(searchParameter)) {
				qry = qry + " and (financial_year_fk like ?"
	...
	public List<Budget> getBudgetList(Budget obj, int startIndex, int offset, String searchParameter) throws Exception {
		List<Budget> objsList = null;
		try {
			String qry ="select budget_id,p.project_id,p.project_name,max(b.financial_year_fk) as financial_year_fk,cast(budget_estimate as CHAR) as budget_estimate,cast(budget_grant as CHAR) as budget_grant, " 
					+ "cast(revised_estimate as CHAR) as revised_estimate,cast(revised_grant as CHAR) as revised_grant,cast(final_estimate as CHAR) as final_estimate,cast(final_grant as CHAR) as final_grant,cast(target as CHAR) as target " 
					+ " from budget b "
					+ "LEFT JOIN financial_year f on b.financial_year_fk = f.financial_year " 
					+ "LEFT JOIN project p on  b.project_id_fk = p.project_id "
			...
			if(!StringUtils.isEmpty(startIndex) && !StringUtils.isEmpty(offset)) {
				qry = qry + " group by budget_id,p.project_id,p.project_name,budget_estimate,budget_grant,revised_estimate,revised_grant,final_estimate,final_grant,target ORDER BY budget_id ASC offset ? rows  fetch next ? rows only";
				arrSize++;
				arrSize++;
			}
			Object[] pValues = new Object[arrSize];
