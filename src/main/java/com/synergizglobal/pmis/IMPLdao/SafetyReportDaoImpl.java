...
public class SafetyReportDaoImpl implements SafetyReportDao{
	...
	public List<Safety> getSafetyReportData(Safety obj) throws Exception {
		List<Safety> objsList = null;
		try {
			String qry = "SELECT safety_id,contract_id_fk,c.contract_short_name,title,d.department_name,description,FORMAT(date,'%d-%m-%y') AS date,location,cast(latitude as CHAR) as latitude,cast(longitude as CHAR) as longitude,reported_by,responsible_person,c.department_fk,"
					...
					+ "FORMAT(investigation_completed,'%d-%m-%Y') AS investigation_completed,corrective_measure_short_term,"
					+ "corrective_measure_long_term,cast(compensation as CHAR) as compensation,"
					+ "FORMAT(payment_date,'%d-%m-%Y') AS payment_date,s.remarks,contract_name,work_id_fk,work_name,work_short_name,"
