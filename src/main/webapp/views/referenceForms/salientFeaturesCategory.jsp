...
<head>
    ...
    <title>Work Salient features category</title>
...
<body>
                ...
                <div class="card-content">
                    <span class="card-title headbg">
                        <div class="center-align bg-m p-2 m-b-5">
                            <h6>Work Salient features category</h6>
                        </div>
                    </span>
                            ...
                            <div class="col m12 s12">
                                <table id="category_table" class="mdl-data-table">
                                    <thead>
                                        <tr>
                                            <th>Category</th>
                                             <c:forEach var="tObj" items="${categoryDetails.tablesList}" >
                                    ...
                                    <tbody>
										<c:forEach var="obj" items="${categoryDetails.dList1}" varStatus="indexs">
											<tr><td>
												<input type="hidden" id="categoryId${indexs.count}" value="${obj.category }"  class="findLengths"/>
												${obj.category }</td>
											<c:forEach var="tObj" items="${categoryDetails.tablesList}" varStatus="index">
												<td><c:forEach var="cObj" items="${categoryDetails.countList}" >
												<c:choose> 
											...
											<td class="last-column "><a onclick="updateRow(${indexs.count})" class="btn waves-effect waves-light bg-m t-c modal-trigger"> <i class="fa fa-pencil" ></i></a>
										 	<c:forEach var="oSbj"  items="${categoryDetails.dList}" varStatus="indexx"> 
    ...
    <div id="addUpdateModal" class="modal">
		<form action="<%=request.getContextPath() %>/add-salient-features-category" id="categoryForm" name="categoryForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header">Add Category <span class="right modal-action modal-close"><span
                    ...
                    <div class="col m8 s12">
                        <div class="row">
                            <div class="input-field col s12 m12">
                                <input id="category_text" name="category" type="text" class="validate" onkeyup="doValidate(this.value)">
                        ...
                        <div class="row">
                            ...
                            <div class="col s12 m6">
                                <div class="center-align m-1">
                                    <a href="<%=request.getContextPath()%>/salient-features-category"
     ...
     <div id="onlyUpdateModal" class="modal">
		 <form action="<%=request.getContextPath() %>/update-salient-features-category" id=updateCategoryForm name="updateCategoryForm" method="post" class="form-horizontal" role="form">
            <div class="modal-content">
                <h5 class="modal-header bg-m">Update Category <span class="right modal-action modal-close" onclick="removeErrorMsg()"><span
                            class="material-icons">close</span></span></h5>
                <div class="row">
                    <div class="col m2 hide-on-small"></div>
                    <div class="col m8 s12">
...
<script src="/pmis/resources/js/dataTables.fixedColumns.min.js"></script>
    ...
    <script>
      ...
      var validator1 = $('#updateCategoryForm').validate({
          	 rules: {
          		 	"value_new": {
      			 		  required: true
          			 }
          	...
          	swal({
      	            ...
      	            if (isConfirm) {
      	               // swal("Deleted!", "Record has been deleted", "success");
      	                $(".page-loader").show();
      	            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-salient-features-category');
      	    	    	$('#getForm').submit();
      	           }else {
