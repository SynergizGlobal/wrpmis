...
@Repository
public class SourceOfFundsDaoImpl implements SourceOfFundsDao{
	...
	public boolean addSourceOfFund(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO source_of_funds"
					...
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select source_of_funds from source_of_funds where source_of_funds NOT IN (?";
						...
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getSource_of_funds();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select t1.name as tname,(select c2.name from sys.columns c2 where c2.object_id=t1.object_id\r\n" + 
					...
					"WHERE t.name = 'source_of_funds' and s.name = 'dbo'),'%')) as COLUMN_NAME,'' as CONSTRAINT_NAME,\r\n" + 
					"'source_of_funds' as REFERENCED_TABLE_NAME,\r\n" + 
					...
					"WHERE t2.name = 'source_of_funds' and s2.name = 'dbo')) as REFERENCED_COLUMN_NAME\r\n" + 
					...
					"left join sys.tables t on t.object_id = c.object_id\r\n" + 
					"left join sys.schemas s on s.schema_id = t.schema_id\r\n" + 
					"WHERE t.name = 'source_of_funds' and s.name = 'dbo'),'%'))\r\n" + 
					"and t1.name !=  'source_of_funds'";
