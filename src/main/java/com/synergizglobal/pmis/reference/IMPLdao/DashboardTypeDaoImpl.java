...
public class DashboardTypeDaoImpl implements DashboardTypeDao{
	...
	public boolean addDashboardType(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO dashboard_type"
					...
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select dashboard_type from dashboard_type where dashboard_type NOT IN (?";
						...
						for (TrainingType aObj : obj.getdList()) {
							pValues[j++] = aObj.getDashboard_type();
							if( objsList1.size() >  p) {
								qry2 = qry2 + ",?";
								p++;
							}
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME as tName,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'dashboard_type' and TABLE_SCHEMA = 'pmis' group by TABLE_NAME";
	...
	private List<TrainingType> getDataDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry = "SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME " + 
					"FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_NAME = 'dashboard_type' and TABLE_SCHEMA = 'pmis'";
	...
	public boolean updateDashBoardType(TrainingType obj) throws Exception {
		...
		try {
			...
			for (TrainingType bObj : obj.getdList()) {
				...
				String updateTableQry = "UPDATE "+bObj.getTable_name()+" SET "+bObj.getColumn_name()+" =:dashboard_type_new WHERE "+bObj.getColumn_name()+"= :dashboard_type_old " ;
