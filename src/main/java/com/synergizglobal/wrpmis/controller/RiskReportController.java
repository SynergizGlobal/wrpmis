...
import com.synergizglobal.wrpmis.Iservice.RiskReportService;
...
public class RiskReportController {
	...
	public List<RiskReport> getAssessmentDateListInRiskReport(@ModelAttribute RiskReport obj) {
		List<RiskReport> assessmentDateList = null;
		try {
			assessmentDateList = riskReportService.getAssessmentDateListInRiskReport(obj);
		}catch (Exception e) {
			e.printStackTrace();
	...
	public ModelAndView generateRiskAnalysisReport(@ModelAttribute RiskReport obj ,HttpServletRequest request,HttpServletResponse response,HttpSession session, RedirectAttributes attributes){
		...
		try{
			...
			boolean flag = generateRiskAnalysisReport(response,currentDate,obj);
		}catch (Exception e) {
			e.printStackTrace();
			logger.error("generateRiskAnalysisReport : " + e.getMessage());
		}
		return model;
     }

	private boolean generateRiskAnalysisReport(HttpServletResponse response, String currentDate, RiskReport obj) {
		...
		try{
			...
			try (ByteArrayOutputStream bos = new ByteArrayOutputStream()){	
				...
				e.printStackTrace();
				logger.error("generateRiskAnalysisReport >> FileNotFoundException occurs.." + e.getMessage());
				flag = false;
		    }	
