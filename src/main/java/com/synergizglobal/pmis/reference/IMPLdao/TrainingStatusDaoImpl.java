...
import com.synergizglobal.pmis.reference.Idao.TrainingStatusDao;
import com.synergizglobal.pmis.reference.model.TrainingType;

@Repository
public class TrainingStatusDaoImpl  implements TrainingStatusDao{
	...
	public List<TrainingType> getTrainingStatusList() throws Exception {
		...
		try {
			String qry ="select status as training_status from training_status ";
	...
	public boolean addTrainingStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO training_status"
	...
	public TrainingType getTrainingStatusDetails(TrainingType obj) throws Exception {
		...
		try {
			String qry ="select status from training_status ";
			...
			if(tablesList.size() > 0) {
				...
				if(objsList1.size() > 0) {
					Object[] pValues  = new Object[objsList1.size()];
					  String qry2 = "select status from training_status where status NOT IN (?";
	...
	private List<TrainingType> getTablesList(TrainingType obj) throws Exception {
		...
		try {
			String qry = "select t1.name as tname,(select c2.name from sys.columns c2 where c2.object_id=t1.object_id\r\n" + 
					...
					"WHERE t.name = 'training_status' and s.name = 'dbo'),'%')) as COLUMN_NAME,'' as CONSTRAINT_NAME,\r\n" + 
					"'training_status' as REFERENCED_TABLE_NAME,\r\n" + 
					...
					"WHERE t2.name = 'training_status' and s2.name = 'dbo')) as REFERENCED_COLUMN_NAME\r\n" + 
					...
					"left join sys.tables t on t.object_id = c.object_id\r\n" + 
					"left join sys.schemas s on s.schema_id = t.schema_id\r\n" + 
					"WHERE t.name = 'training_status' and s.name = 'dbo'),'%'))\r\n" + 
					"and t1.name !=  'training_status'";
	...
	public boolean updateTrainingStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String  updatereferenceTableQry = "UPDATE training_status SET status= :status_new WHERE status= :status_old " ;
	...
	public boolean deleteTrainingStatus(TrainingType obj) throws Exception {
		...
		try {
			...
			String deleteQry ="DELETE from training_status WHERE status= :status; ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
