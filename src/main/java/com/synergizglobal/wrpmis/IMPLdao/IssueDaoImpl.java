...
import com.synergizglobal.wrpmis.Idao.IssueDao;
...
public class IssueDaoImpl implements IssueDao {
	...
	public List<Issue> getIssuesList(Issue obj) throws Exception {
		...
		try {
			String qry = "SELECT "
				    + "i.issue_id,i.contract_id_fk,"
				    + "CONCAT(i.zonal_railway_fk,' - ',d.department_name) AS other_organization," 
				    + "d.department_name,c.contract_short_name,title,"
				    + "FORMAT(date,'dd-MMM-yy') AS date,location,reported_by,responsible_person,"
				    + "u.department_fk,priority_fk,category_fk,status_fk,corrective_measure,"
				    + "FORMAT(resolved_date,'dd-MMM-yy') AS resolved_date,escalated_to,i.remarks,"
				    + "contract_name,project_id_fk,project_name,i.zonal_railway_fk,r.railway_name,"
				    + "u2.designation AS responsible_person_designation,"
				    + "u3.designation AS escalated_to_designation,"
				    + "railway_name,FORMAT(assigned_date,'dd-MMM-yy') AS assigned_date,"
				    + "c.hod_user_id_fk,c.dy_hod_user_id_fk,created_by_user_id_fk,"
				    + "FORMAT(i.created_date,'dd-MMM-yy') AS created_date,"
				    + "FORMAT(escalation_date,'dd-MMM-yy') AS escalation_date,"
				    + "other_org_resposible_person_name,other_org_resposible_person_designation,"
				    + "description,i.modified_by,FORMAT(i.modified_date,'dd-MM-yyyy') AS modified_date "
	...
	public List<Issue> getRailwayList() throws Exception {
		List<Issue> objsList = null;
		try {
			String qry = "SELECT railway_id,railway_name from railway WHERE railway_id <> 'Con' and railway_id='WR' ORDER BY case when railway_id='WR' then 1" + 
	...
	public boolean addIssue(Issue obj) throws Exception {
		...
		try {
			...
			String qry = "INSERT INTO issue"
					...
					+ "VALUES "
					+ "(:contract_id_fk,:title1,:date,:location,:reported_by,:responsible_person,:"
					+ "priority_fk,:category_fk,:status_fk,:assigned_date,:corrective_measure,:resolved_date,:escalated_to,:remarks,"
	...
	public Issue getIssue(Issue obj) throws Exception {
		Issue iObj = null;
		try {
			String qry = "select issue_id,contract_id_fk,title,FORMAT(date,'dd-MM-yyyy') AS date,location,reported_by,responsible_person,u.department_fk,"
	...
	public void sendEmailWithIssueStatusAlert1(String issue_id, String issue_status,String issueDetailsUrl, String today_date, String current_year) {
	    try {
			String emailsQry = "select i.issue_id,i.contract_id_fk,i.status_fk,i.reported_by,c.contract_short_name,c.contract_name,i.category_fk,i.priority_fk,i.title,i.location,i.corrective_measure,i.remarks,"
					+ "u2.designation as responsible_person_designation,u3.designation as escalated_to_designation,"
	...
	public List<Issue> getDepartmentsListFilter(Issue obj) throws Exception {
		List<Issue> objsList = null;
		try {
			String qry = "SELECT u.department_fk,department,department_name " + "from issue i "
	...
	public List<Issue> getOtherOrganizationsList() throws Exception {
		List<Issue> objsList = null;
		try {
			String qry = "SELECT issue_other_organization as other_organization,issue_other_organization from issue_other_organization";
			objsList = jdbcTemplate.query(qry, new BeanPropertyRowMapper<Issue>(Issue.class));
		} catch (Exception e) {
			throw new Exception(e);
		}
	...
	public List<Issue> getPendingIssues(Issue obj) throws Exception {
		List<Issue> objsList = null;
		try {
			String qry = "select issue_id,contract_id_fk,d.department_name,c.contract_short_name,title,FORMAT(date,'dd-MMM-yy') AS date,location,reported_by,responsible_person,u.department_fk,"
	...
	public List<Issue> getUnresolvedIssues() {
		...
		String qry = "select i.issue_id,zonal_railway_fk,description,date,contractor_name,i.contract_id_fk,i.status_fk,i.reported_by,c.contract_short_name,c.contract_name,i.category_fk,i.priority_fk,i.title,i.location,i.corrective_measure,i.remarks,u2.designation as responsible_person_designation,u3.designation as escalated_to_designation,u2.email_id as responsible_person_email_id,u3.email_id as escalated_to_email_id,u4.email_id as contract_hod_email_id,u5.email_id as contract_dyhod_email_id,u5.user_name as dyhod_name,u4.user_name as hod_name,i.responsible_person as responsible_person_user_id,i.escalated_to as escalated_to_user_id,c.hod_user_id_fk as contract_hod_user_id,c.dy_hod_user_id_fk as contract_dyhod_user_id,u1.email_id as created_by_email_id,i.created_by_user_id_fk,other_org_resposible_person_name,other_org_resposible_person_designation,(select top 1 email_id from issue i\r\n" + 
				...
				"(select top 1 user_name as cmd_name from [user] where designation like '%CMD%') as cmd_name,i.created_date,(select email_id from [user] where user_name=c1.contractor_name) as contractor_email,(select top 1 comment from issue_history where issue_id_fk=1 and comment is not null order by id desc\r\n" + 
				") as comment  \r\n" + 
				"\r\n" + 
				"\r\n" + 
				"\r\n" + 
	...
	@Override
	public boolean sendErrorReminderEmail(Issue issue, int daysPending) {
	    try {
	        String subject = "Failed---"+issue.getIssue_id();
	        String body = getEmailBody(daysPending, issue);
	        String to = getRecipientEmail(daysPending, issue);
			...
			if (!StringUtils.isEmpty(to)) {
				EMailSender emailSender = new EMailSender();
				logger.error("sendEmailWithIssueStatusAlert() >> Sending mail to " + to + ": Start ");
				emailSender.sendErrorEmailWithIssueStatusAlert(mail, issue, today_date, current_year);
				logger.error("sendEmailWithIssueStatusAlert() >> Sending mail to " + to + ": end ");
			}	        
	        return true;
	    } catch (Exception e) {
