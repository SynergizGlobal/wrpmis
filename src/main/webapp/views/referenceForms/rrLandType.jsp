...
<head>
    ...
    <style>
        ...
        .mdl-data-table tfoot tr {
            background-color: #463409 !important;
        }
		input[type=number]:not(.browser-default):focus:not([readonly]),
		input[type=text]:not(.browser-default):focus:not([readonly]),
		input[type=search]:not(.browser-default):focus:not([readonly]),
		textarea.materialize-textarea:focus:not([readonly])   {
		    border-bottom: 1px solid #463409 !important;
		    box-shadow: 0 1px 0 0 #463409 !important;
		}
                        ...
                        <div class="row no-mar">
                            <div class="col m12 s12">
                                <table id="rr_land_type_table" class="mdl-data-table">
                                    ...
                                    <tbody>
										<c:forEach var="obj" items="${rrLandTypeDetails.dList1}" varStatus="indexs">
											<tr><td>
												<input type="hidden" id="land_typeId${indexs.count}" value="${obj.land_type }" />
												${obj.land_type }</td>
											<c:forEach var="tObj" items="${rrLandTypeDetails.tablesList}" varStatus="index">
												<td><c:forEach var="cObj" items="${rrLandTypeDetails.countList}" >
												<c:choose> 
													    <c:when test="${tObj.tName eq cObj.tName }"> 
													    		<c:choose>  
																    <c:when test="${cObj.land_type eq obj.land_type }"> 
																      	 ( ${cObj.count } )   
																    </c:when>  
																    <c:otherwise>  
																    </c:otherwise>   
											...
											<td class="last-column "><a onclick="updateRow(${indexs.count})" class="btn waves-effect waves-light bg-m t-c modal-trigger " href="#"> <i class="fa fa-pencil" ></i></a>
										 	<c:forEach var="oSbj"  items="${rrLandTypeDetails.dList}" varStatus="indexx"> 
												<c:choose>  
												    <c:when test="${oSbj.land_type eq obj.land_type }"> 
												      	<a onclick="deleteRow('${ oSbj.land_type }');" id="${indexx.count}" class="btn waves-effect waves-light bg-s t-c modal-trigger"><i class="fa fa-trash"></i>
                        ...
                        <div class="row">
                            <div class="col s12 m6">
                                <div class="center-align m-1">
                                    <button style="width: 100%;" onclick="addRRLandType()"
                             ...
                             <div class="col s12 m6">
                                <div class="center-align m-1">
                                  <!--   <button
                                        ...
                                        <a href="<%=request.getContextPath()%>/rr-land-type"
                ...
                <div class="row">
                    ...
                    <div class="col m8 s12">
                        ...
                        <div class="row">
                            <div class="col s12 m6">
                                <div class="center-align m-1">
                                    <button style="width: 100%;" onclick="updateRRLandType()"
    ...
    <script>
        $(document).ready(function () {
          ...
          /*   // adding table data into table start
            ...
            $('#rr_land_type_table tbody').append(table_text);
            // adding table data into table ends */

            var table = $('#rr_land_type_table').DataTable({
                columnDefs: [
                    {
                        targets: [0],
     ...
     function addRRLandType(){
        	 if(validator.form()){ 
    			$(".page-loader").show();
    			$("#addUpdateModal").modal();
    			document.getElementById("addRRLandTypeForm").submit();	
         }
     }
     
     function updateRRLandType(){
    	 if(validator1.form()){ 
			$(".page-loader").show();
			$("#onlyUpdateModal").modal();
			document.getElementById("updateRRLandTypeForm").submit();	
     }
 }
 
     var validator =	$('#addRRLandTypeForm').validate({
     	 rules: {
     		 "land_type": {
			 		  required: true
     		 }
	        	 ...
	        	 if(element.attr("id") == "rr_land_type_text" ){
				     document.getElementById("land_typeError").innerHTML="";
			 	     error.appendTo('#land_typeError');
				 }
	        }
     	
     });
     
     var validator1 =  $('#updateRRLandTypeForm').validate({
       	 rules: {
       		 "value_new": {
    			 		  required: true
       		 }
        ...
        function updateRow(no) {
    	      var land_type = $('#land_typeId'+no).val();
    	      $('#value_old').val($.trim(land_type))
    	      $('#onlyUpdateModal').modal('open');
    	      $('#onlyUpdateModal #value_new').val($.trim(land_type)).focus();
    	  }
    	    	...
    	    	swal({
    		            ...
    		            if (isConfirm) {
    		               // swal("Deleted!", "Record has been deleted", "success");
    		            	$('#getForm').attr('action', '<%=request.getContextPath()%>/delete-rr-land-type');
    		    	    	$('#getForm').submit();
    		           }else {
